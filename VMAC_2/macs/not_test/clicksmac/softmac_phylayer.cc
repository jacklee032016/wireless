/*
 * softmac_phylayer.{cc,hh} -- interface between CU VMAC API and Click
 * Michael Neufeld
 * Jeff Fifield
 *
...
SoftmacPHYLayer::~SoftmacPHYLayer()
{
    ...
    vmac_free_phy(_defaultphy);
}

SoftmacPHYLayer*
SoftmacPHYLayer::make(vmac_phy_t *phyinfo)
{
...
{
    vmac_phy_t *p = vmac_get_phy_by_name(phy_instance_name);
...
SoftmacPHYLayer::CreatePacket(int size) 
{
    ...
    newskb = (_phyinfo->phy_alloc_skb)(_phyinfo->phy_private,size);
    if (newskb) {
...
SoftmacPHYLayer::DestroyPacket(Packet* p)
{
    ...
    (_phyinfo->phy_free_skb)(_phyinfo->phy_private,p->skb());
}
...
SoftmacPHYLayer::GetPacketTransmitDuration(Packet* p)
{
    ...
    return (_phyinfo->phy_get_duration)(_phyinfo->phy_private,p->skb());
}

u_int32_t
SoftmacPHYLayer::GetTransmitLatency()
{
    return (_phyinfo->phy_get_txlatency)(_phyinfo->phy_private);
}
