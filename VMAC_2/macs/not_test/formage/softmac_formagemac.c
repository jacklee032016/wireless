 ...
 */
static vmac_layer_t the_formagemac;
...
formagemac_make_procfs_entries(struct formagemac_instance *inst)
{
	...
	for (i=0; (formagemac_inst_proc_entries[i].name && formagemac_inst_proc_entries[i].name[0]); i++) {
	    ...
	    strncpy(proc_data->name, formagemac_inst_proc_entries[i].name, VMAC_NAME_SIZE);
...
static int
formagemac_mac_attach(void *me, vmac_phy_t *phyinfo)
{
    //printk("%s\n", __func__);
...
formagemac_new_instance (void *layer_priv)
{
	...
	snprintf(inst->macinfo->name, VMAC_NAME_SIZE, "%s%d", the_formagemac.name, inst->id);

	/* override some macinfo functions */
	/* the rest remain pointed at the default "do nothing" functions */
	inst->macinfo->mac_tx = formagemac_mac_packet_tx;
	inst->macinfo->rx_packet = formagemac_mac_packet_rx;
	inst->macinfo->attach_phy = formagemac_mac_attach;
	inst->macinfo->detach_phy = formagemac_mac_detach;
	inst->macinfo->set_rx_func = formagemac_mac_set_netif_rx_func;
...
/* called by softmac_core when a formagemac vmac_mac_t
...
softmac_formagemac_init(void)
{
    ...
    strncpy(the_formagemac.name, formagemac_name, VMAC_NAME_SIZE);
    the_formagemac.new_instance = formagemac_new_instance;
    the_formagemac.destroy_instance = formagemac_free_instance;
    vmac_layer_register(&the_formagemac);
...
softmac_formagemac_exit(void)
{
    ...
    vmac_layer_unregister((vmac_layer_t *)&the_formagemac);
}
