...
static int cu_softmac_mac_attach_cheesymac(void* handle, vmac_phy_t* phyinfo);

/***  Tell the MAC layer to detach from the specified PHY layer.  */
static int cu_softmac_mac_detach_cheesymac(void* handle);

static int cu_softmac_mac_set_rx_func_cheesymac(void* handle, mac_rx_func_t rxfunc,void* rxpriv);

static int cu_softmac_mac_set_unload_notify_func_cheesymac(void* handle,
						mac_unload_notify_func_t unloadfunc,	void* unloadpriv);

/*
** Declarations of internal functions ("private" members)
*/
static void cheesymac_destroy_instance(void* mypriv);
static CHEESYMAC_INSTANCE *cheesymac_create_instance(vmac_mac_t* macinfo, CU_SOFTMAC_CHEESYMAC_PARAMETERS* params);
...
MODULE_PARM_DESC(cheesymac_defaultphy, "Network interface to use for VMAC PHY layer");

static int  multimac_set_rx_func(char *name, void* mydata, mac_rx_func_t rxfunc, void* rxpriv)
{
	int result = -1;
	...
	if (inst)
	{
		...
		for(i=0; i<MULTIMAC_MAX_MACS; i++)
		{
			if(inst->macs[i])
			{
				if(strncmp(name, inst->macs[i]->name, VMAC_NAME_SIZE)==0)
				{
					if(rxfunc)
...
static int multimac_set_tx_func(char *name, void* mydata,
		    mac_rx_func_t txfunc, void* txpriv)
{
	int result = -1;
	CHEESYMAC_INSTANCE* inst = mydata;
	if (inst)
...
static int multimac_netif_rxhelper(void* priv, struct sk_buff* packet, int intop)
{
	...
	if (inst && inst->myphy)
	{
		...
		if (!inst->raw_mode && vmac_ath_issoftmac(inst->myphy->phy_private, packet))
			packet = vmac_ath_decapsulate(inst->myphy->phy_private, packet);
...
static int multimac_netif_txhelper(vnet_handle nif,void* priv, struct sk_buff* packet)
{
	CHEESYMAC_INSTANCE* inst = priv;
	...
	if (inst)
	{
		...
		* instigated by a softirq in the bottom half.     */
		...
		for (i=0; i<MULTIMAC_MAX_MACS; i++)
		{
			...
			{
				if(strncmp(inst->preferred, "*", 1)==0 || strncmp(inst->preferred, inst->macs[i]->name, VMAC_NAME_SIZE)==0)
				{
					struct sk_buff *skb = skb_copy(packet, GFP_ATOMIC);
    ...
    for(i=0; i<MULTIMAC_MAX_MACS->; i++)
    {
    	if(inst->macs[i])
	{
		if(strncmp(currentmacname, (inst->macs[i])->name, VMAC_NAME_SIZE)==0)
    			break;
	}
    }
	...
	if (!strncmp(currentmacname, (inst->macs[0])->name, VMAC_NAME_SIZE)) {
	    strncpy(currentmacname, (inst->macs[1])->name, VMAC_NAME_SIZE);
	} else {
	    strncpy(currentmacname, (inst->macs[0])->name, VMAC_NAME_SIZE);
	}
    }
...
** VMAC Netif stuff
*/
static int cheesymac_netif_txhelper(vnet_handle nif, void* priv, struct sk_buff* packet) 
{
	CHEESYMAC_INSTANCE* inst = priv;
	if (inst)
	{/** Call the cheesymac tx function. The "intop" indicator
...
static int cheesymac_netif_unload_helper(vnet_handle netif, void* priv) 
{
	CHEESYMAC_INSTANCE* inst = priv;
	if (inst)
	{
...
static int cheesymac_create_and_attach_netif(void *mypriv)
{
	...
	if (checknet)
	{
		...
		dev_put(checknet);
		/*vmac_vnetif_set_tx_callback(inst->netif, cheesymac_netif_txhelper, (void *)inst);*/

		vmac_vnetif_set_tx_callback(inst->netif,multimac_netif_txhelper, (void *)inst);
	}
	else
	{
		printk(KERN_DEBUG "MultiMAC: Creating %s\n", macinfo->name);
		/*inst->netif = vmac_create_vnetif(macinfo->name, 0,cheesymac_netif_txhelper,	inst);*/
		inst->netif = vmac_create_vnetif(macinfo->name, 0,multimac_netif_txhelper,inst);
	}

	if (inst->netif)
	{//printk(KERN_DEBUG "MultiMAC: Setting mac unload notify func\n");
		(macinfo->set_unload_notify_func)(inst,	 vmac_vnetif_detach, inst->netif);
		//printk(KERN_DEBUG "MultiMAC: Setting netif unload callback func\n");
		vmac_netif_set_unload_callback(inst->netif,  cheesymac_netif_unload_helper, inst);
		(macinfo->set_rx_func)(inst, vmac_vnetif_rx, inst->netif);
	}
	else
	{
...
static struct sk_buff* multimac_phy_alloc_skb(void *mydata, int len)
{
	...
	if (inst->myphy)
		skb = inst->myphy->phy_alloc_skb(inst->myphy->phy_private, len);
...
static int multimac_phy_sendpacket(void* mydata, int max_inflight, struct sk_buff* packet)
{
	...
	if (inst && inst->myphy)
	{
		...
		* we taking an interrupt right now?     */
		...
		{
			if (!inst->raw_mode)
				vmac_ath_encapsulate(inst->myphy->phy_private, packet);
			...
			{
				printk(KERN_ALERT "VMAC MultiMAC: tasklet packet tx failed: %d\n",txresult);
		...
		if (intop)
		{
			...
			* of an interrupt and the place for time-critical tasks to occur.       */
			...
			else if (packet)
			{
				...
				if (VMAC_PHY_SENDPACKET_OK != txresult)
				{
					printk(KERN_ALERT "VMAC MultiMAC: top half packet tx failed: %d\n",txresult);
				}
				status = MAC_TX_OK;
			}
		}
...
static int cu_softmac_mac_work_cheesymac(void* mydata, int intop) 
{
	...
	if (inst)
	{
		...
		while ((skb = skb_dequeue(&(inst->tx_skbqueue))))
		{
			...
			if (VMAC_PHY_SENDPACKET_OK != txresult)
			{
				printk(KERN_ALERT "VMAC MultiMAC: work packet tx failed: %d\n",txresult);
			}
		}
						     ...
						     CU_SOFTMAC_CHEESYMAC_PARAMETERS* params) 
{
	...
	* make it part of cheesymac_instance_list linked list, return it   */
	...
	if (inst)
	{
		...
		inst->passedup = 0;
		inst->gotthere = 0;
		inst->preferred = kmalloc(VMAC_NAME_SIZE, GFP_KERNEL);
		...
		macinfo->rx_packet = multimac_netif_rxhelper;
		...
		inst->multimac_fake_phy = vmac_alloc_phy();
		inst->multimac_fake_phy->phy_private = inst;
		snprintf(inst->multimac_fake_phy->name, VMAC_NAME_SIZE, "%s_fake_phy", macinfo->name);
		inst->multimac_fake_phy->phy_sendpacket = multimac_phy_sendpacket;
		inst->multimac_fake_phy->phy_alloc_skb = multimac_phy_alloc_skb;

		// RR-Thing - Remove!    
		currentmacname = kmalloc(VMAC_NAME_SIZE, GFP_KERNEL);
...
static void cheesymac_destroy_instance(void* mypriv) 
{
	...
	if (inst)
	{
		...
		vmac_free_phy(inst->multimac_fake_phy);
		write_unlock(&(inst->mac_busy));
	...
	if (procdata && procdata->inst)
	{
		...
		switch (procdata->entryid)
		{
			...
			case CHEESYMAC_INST_PROC_PREFERRED:
				...
				kdata[result-1] = 0;
				write_lock(&(inst->mac_busy));
				strncpy(inst->preferred, kdata, VMAC_NAME_SIZE);
				write_unlock(&(inst->mac_busy));
				break;
			...
			case CHEESYMAC_INST_PROC_ADDMAC:
			{
				...
				mac = vmac_layer_new_instance(kdata);  
				mac = vmac_get_mac(mac);
				...
				for(i=0; i<MULTIMAC_MAX_MACS; i++)
				{
					if(inst->macs[i]==NULL)
					{
						...
						strncpy((inst->macs[i])->name, mac->name, VMAC_NAME_SIZE);
						(inst->macs[i])->mac = mac;
						(inst->macs[i])->myrxfunc = mac->rx_packet;
						(inst->macs[i])->mytxfunc = mac->mac_tx;
						mac->set_rx_func(mac->mac_private, vmac_vnetif_rx,inst->netif);
						mac->attach_phy(mac->mac_private,inst->multimac_fake_phy);
			...
			case CHEESYMAC_INST_PROC_DELETEMAC:
				...
				if((inst->runningmacs)>=1)
				{
					...
					for(i=0; i<MULTIMAC_MAX_MACS; i++)
					{
						if(inst->macs[i]!=NULL)
						{
							if(strncmp((inst->macs[i])->name, kdata, VMAC_NAME_SIZE)==0)
							{
								printk("Removing mac layer %s.\n", (inst->macs[i])->name);

								vmac_mac_t *mac = (inst->macs[i])->mac;
								mac->set_rx_func(mac->mac_private, 0, 0);
								mac->detach_phy(mac->mac_private);
								vmac_free_mac(mac);
								kfree(inst->macs[i]);
								inst->macs[i] = NULL;
								inst->runningmacs--;
...
static int cu_softmac_mac_attach_cheesymac(void* handle, vmac_phy_t* phyinfo)
{
	...
	if (inst && phyinfo)
	{
		//printk(KERN_DEBUG "VMAC MultiMAC: Attaching to PHY -- getting lock\n");
		write_lock(&(inst->mac_busy));
		//printk(KERN_DEBUG "VMAC MultiMAC: Attaching to PHY -- got lock\n");

		if (inst->myphy)
		{
			/** Already attached -- bail out    */
			printk(KERN_ALERT "VMAC MultiMAC: Attempting to attach to a phy layer while still attached to a phy layer!\n");
			result = -1;
		}
		else
		{
		...
		//printk(KERN_DEBUG "VMAC MultiMAC: Unlocking MAC\n");
		write_unlock(&(inst->mac_busy));
	}
	else
	{
		printk(KERN_ALERT "VMAC MultiMAC: Invalid MAC/PHY data on attach!\n");
		result = -1;
	}
...
static int cu_softmac_mac_set_rx_func_cheesymac(void* mydata,mac_rx_func_t rxfunc, void* rxpriv)
{
	int result = 0;
	CHEESYMAC_INSTANCE* inst = mydata;
...
void cu_softmac_cheesymac_get_default_params(CU_SOFTMAC_CHEESYMAC_PARAMETERS* params)
{
	...
	{
		printk(KERN_DEBUG "VMAC MultiMAC: Called get_default_params with null parameters!\n");
	}
}
...
void cu_softmac_cheesymac_set_default_params(CU_SOFTMAC_CHEESYMAC_PARAMETERS* params)
{
	...
	{
		printk(KERN_DEBUG "VMAC MultiMAC: Called set_default_params with null parameters!\n");
	}
}
...
void cu_softmac_cheesymac_get_instance_params(void* macpriv, CU_SOFTMAC_CHEESYMAC_PARAMETERS* params)
{
	...
	{
		printk(KERN_DEBUG "VMAC MultiMAC: Called get_instance_params with bad data!\n");
	}
}
...
void cu_softmac_cheesymac_set_instance_params(void* macpriv, CU_SOFTMAC_CHEESYMAC_PARAMETERS* params)
{
	...
	{
		printk(KERN_DEBUG "VMAC MultiMAC: Called set_instance_params with bad data!\n");
	}
}
...
static int __init softmac_cheesymac_init(void)
{
	...
	strncpy(the_cheesymac.name, "multimac", VMAC_NAME_SIZE);
