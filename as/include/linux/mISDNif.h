/* $Id: mISDNif.h,v 1.1.1.1 2006/11/29 08:56:06 lizhijie Exp $
 *
 */
...
#define MISDN_REVISION		"$Revision: 1.1.1.1 $"
#define MISDN_DATE		"$Date: 2006/11/29 08:56:06 $"
...
/* status of mISDNdevice */
#define FLG_mISDNPORT_BUSY		1
#define FLG_mISDNPORT_ENABLED		2
#define FLG_mISDNPORT_BLOCK		3	/* blocked :can not be used */
...
struct _mISDNobject
{
	...
	mISDN_pid_t		DPROTO;	/* D Channel Protocol */
	mISDN_pid_t		BPROTO;		/* B Channel Protocol */
	ctrl_func_t		*own_ctrl;
	ctrl_func_t		*ctrl;
	struct list_head	ilist;		/* private data in every mISDNobject, such as layer1_t in isdnl1 */
	struct module	*owner;
};

/* the interface between two mISDNinstances 
* this object can only exist in mISDNinstance or mISDNport
*/
struct _mISDNif
{
	if_func_t			*func;
...
struct _mISDNinstance
{
	...
	mISDNstack_t		*st;
	mISDNobject_t	*obj;
	void				*data;		/* data or structure used in mISDNobject in this instance, such as dtmf_t */
	mISDNif_t		up;
	mISDNif_t		down;
	lock_func_t		*lock;
