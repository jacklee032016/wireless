/* 
 * $Id: hfc_multi.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
...
static const char *hfcmulti_revision = "$Revision: 1.1.1.1 $";

static int HFC_cnt;

static AS_ISDNobject_t	HFCM_obj;
...
{
	register AS_ISDN_HWlock_t *lock = &((hfc_multi_t *)data)->lock;
...
init_chip(hfc_multi_t *hc)
{
	...
	if (test_bit(HFC_CHIP_EXRAM_128, &hc->chip)) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: changing to 128K extenal RAM\n", __FUNCTION__);
	...
	if (test_bit(HFC_CHIP_EXRAM_512, &hc->chip)) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: changing to 512K extenal RAM\n", __FUNCTION__);
		hc->hw.r_ctrl |= V_EXT_RAM;
		hc->hw.r_ram_sz = 2;
		hc->Flen = 0x20;
		hc->Zmin = 0xc0;
	...
	if (test_bit(HFC_CHIP_PCM_SLAVE, &hc->chip)) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: setting PCM into slave mode\n", __FUNCTION__);
	} else {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: setting PCM into master mode\n", __FUNCTION__);
	...
	if (test_bit(HFC_CHIP_CLOCK2, &hc->chip)) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: setting double clock\n", __FUNCTION__);
	...
	if (debug & DEBUG_HFCMULTI_INIT)
		printk(KERN_ERR  "HFC_multi F0_CNT %ld after status ok\n", val);
	unlock_dev(hc);
	...
	lock_dev(hc, 0);
	if (debug & DEBUG_HFCMULTI_INIT)
		printk(KERN_ERR  "HFC_multi F0_CNT %ld after %dms\n", val2, cnt);
	if (val2 < val+4) {
	...
	if (test_bit(HFC_CHIP_DTMF, &hc->chip)) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: enabling DTMF detection for all B-channel\n", __FUNCTION__);
	...
	if (hc->masterclk >= 0) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: setting ST master clock to port %d (0..%d)\n", __FUNCTION__, hc->masterclk, hc->type-1);
...
hfcmulti_dtmf(hfc_multi_t *hc)
{
	...
	if (debug & DEBUG_HFCMULTI_DTMF)
		printk(KERN_ERR  "%s: dtmf detection irq\n", __FUNCTION__);
	ch = 0;
	while(ch < 32) {
		...
		if (debug & DEBUG_HFCMULTI_DTMF)
			printk(KERN_ERR  "%s: dtmf channel %d:", __FUNCTION__, ch);
		dtmf = 1;
		co = 0;
		while(co < 8) {
...
next_frame:
	if (hdlc) {
		...
		while (f2 != (temp=HFC_inb_(hc, A_F2))) {
			if (debug & DEBUG_HFCMULTI_FIFO)
				printk(KERN_ERR  "%s: reread f2 because %d!=%d\n", __FUNCTION__, temp, f2);
	...
	while(z2 != (temp=(HFC_inw_(hc, A_Z2) - hc->Zmin))) {
		if (debug & DEBUG_HFCMULTI_FIFO)
			printk(KERN_ERR  "%s: reread z2 because %d!=%d\n", __FUNCTION__, temp, z2);
		...
		if (z1 == z2) { /* empty */
			...
			if (!hdlc && txpending && slot_tx>=0) {
				if (debug & DEBUG_HFCMULTI_MODE)
					printk(KERN_ERR  "%s: reconnecting PCM due to no more FIFO data: channel %d slot_tx %d\n", __FUNCTION__, ch, slot_tx);
	...
	if (!hdlc && !txpending && slot_tx>=0) {
		if (debug & DEBUG_HFCMULTI_MODE)
			printk(KERN_ERR  "%s: disconnecting PCM due to FIFO data: channel %d slot_tx %d\n", __FUNCTION__, ch, slot_tx);
		/* disconnect slot */
	...
	if (ii-i > Zspace)
		ii = Zspace+i;
	if (debug & DEBUG_HFCMULTI_FIFO) {
		printk(KERN_ERR  "%s: fifo(%d) has %d bytes space left (z1=%04x, z2=%04x) sending %d of %d bytes %s\n",
...
next_frame:
	...
	if (hdlc) {
		...
		while (f1 != (temp=HFC_inb_(hc, A_F1))) {
			if (debug & DEBUG_HFCMULTI_FIFO)
				printk(KERN_ERR  "%s: reread f1 because %d!=%d\n", __FUNCTION__, temp, f1);
	...
	if (hdlc) {
		if (debug & DEBUG_HFCMULTI_FIFO)
			printk(KERN_ERR  "%s: fifo(%d) reading %d bytes (z1=%04x, z2=%04x) HDLC %s (f1=%d, f2=%d) got=%d\n",
				__FUNCTION__, ch, Zsize, z1, z2, (f1==f2)?"fragment":"COMPLETE", f1, f2, Zsize+*idx);
		/* HDLC */
		ii = Zsize;
		if ((ii + *idx) > max) {
			if (debug & DEBUG_HFCMULTI_FIFO)
				printk(KERN_ERR  "%s: hdlc-frame too large.\n", __FUNCTION__);
			*idx = 0;
			HFC_outb_(hc, R_INC_RES_FIFO, V_RES_F);
			HFC_wait_(hc);
			return;
		...
		*idx += ii;
		if (f1 != f2) {
			...
			if (*idx < 4) {
				if (debug & DEBUG_HFCMULTI_FIFO)
					printk(KERN_ERR  "%s: Frame below minimum size\n", __FUNCTION__);
				return;
			}
			...
			if (dch) {
				if (debug & DEBUG_HFCMULTI_FIFO)
					printk(KERN_ERR  "%s: sending D-channel frame to user space.\n", __FUNCTION__); 
		...
		if (debug & DEBUG_HFCMULTI_FIFO)
			printk(KERN_ERR  "%s: fifo(%d) reading %d bytes (z1=%04x, z2=%04x) TRANS\n",
				...
				if (hc->created[hc->chan[ch].port]) {
					...
					if (hc->chan[ch].nt_timer > -1) {
						if (!(--hc->chan[ch].nt_timer)) {
							dchannel_sched_event(dch, D_L1STATECHANGE);
							if (debug & DEBUG_HFCMULTI_STATE)
								printk(KERN_ERR  "%s: nt_timer at state %x\n", __FUNCTION__, dch->ph_state);
						}
					}
				}
				if ((bch = hc->chan[ch].bch))
			...
			if (hc->created[0]) {
				if (test_bit(HFC_CFG_REPORT_LOS, &hc->chan[16].cfg)) {
					if (debug & DEBUG_HFCMULTI_SYNC)
						printk(KERN_ERR  "%s: (id=%d) E1 got LOS\n", __FUNCTION__, hc->id);
					/* LOS */
				...
				if (test_bit(HFC_CFG_REPORT_AIS, &hc->chan[16].cfg)) {
					if (debug & DEBUG_HFCMULTI_SYNC)
						printk(KERN_ERR  "%s: (id=%d) E1 got AIS\n", __FUNCTION__, hc->id);
					/* AIS */
				...
				if (test_bit(HFC_CFG_REPORT_SLIP, &hc->chan[16].cfg)) {
					if (debug & DEBUG_HFCMULTI_SYNC)
						printk(KERN_ERR  "%s: (id=%d) E1 got SLIP (RX)\n", __FUNCTION__, hc->id);
					/* SLIP */
				...
				switch(hc->chan[16].sync) {
					...
					if ((temp&0x60) == 0x60) {
						if (debug & DEBUG_HFCMULTI_SYNC)
							printk(KERN_ERR  "%s: (id=%d) E1 now in clock sync\n", __FUNCTION__, hc->id);
					...
					if ((temp&0x60) != 0x60) {
						if (debug & DEBUG_HFCMULTI_SYNC)
							printk(KERN_ERR  "%s: (id=%d) E1 lost clock sync\n", __FUNCTION__, hc->id);
						hc->chan[16].sync = 0;
						break;
					}
					check_framesync:
					...
					if (temp == 0x27) {
						if (debug & DEBUG_HFCMULTI_SYNC)
							printk(KERN_ERR  "%s: (id=%d) E1 now in frame sync\n", __FUNCTION__, hc->id);
						hc->chan[16].sync = 2;
					}
					break;
					...
					if ((temp&0x60) != 0x60) {
						if (debug & DEBUG_HFCMULTI_SYNC)
							printk(KERN_ERR  "%s: (id=%d) E1 lost clock & frame sync\n", __FUNCTION__, hc->id);
						hc->chan[16].sync = 0;
						break;
					}
					temp = HFC_inb_(hc, R_RX_STA0);
					if (temp != 0x27) {
						if (debug & DEBUG_HFCMULTI_SYNC)
							printk(KERN_ERR  "%s: (id=%d) E1 lost frame sync\n", __FUNCTION__, hc->id);
						hc->chan[16].sync = 1;
					}
					break;
				}
...
mode_hfcmulti(hfc_multi_t *hc, int ch, int protocol, int slot_tx, int bank_tx, int slot_rx, int bank_rx)
{
	...
	if (debug & DEBUG_HFCMULTI_MODE)
		printk(KERN_ERR  "%s: channel %d protocol %x slot %d bank %d (TX) slot %d bank %d (RX)\n", __FUNCTION__, ch, protocol, slot_tx, bank_tx, slot_rx, bank_rx);
	...
	switch (protocol) {
		...
		default:
		printk(KERN_ERR  "%s: protocol not known %x\n", __FUNCTION__, protocol);
 ...
 */
static int
hfcmulti_l1hw(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	dchannel_t	*dch;
...
//		}
...
#endif
			default:
			printk(KERN_ERR  "%s: unknown PH_SIGNAL info %x\n", __FUNCTION__, hh->dinfo);
			ret = -EINVAL;
		}
		dch->inst.unlock(hc);
	} else 
	if (hh->prim == (PH_CONTROL | REQUEST)) {
		...
		switch (hh->dinfo) {
			...
			case HW_DEACTIVATE:
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: HW_DEACTIVATE\n", __FUNCTION__);
			...
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: HW_PCM_CONN slot %d bank %d (TX) slot %d bank %d (RX)\n", __FUNCTION__, slot_tx, bank_tx, slot_rx, bank_rx);
			...
			case HW_PCM_DISC:
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: HW_PCM_DISC\n", __FUNCTION__);
			hfcmulti_pcm(hc, dch->channel, -1, -1, -1, -1); 
			break;

			default:
			printk(KERN_ERR  "%s: unknown PH_CONTROL info %x\n", __FUNCTION__, hh->dinfo);
			ret = -EINVAL;
		}
		dch->inst.unlock(hc);
	} else
	if (hh->prim == (PH_ACTIVATE | REQUEST)) {
		if (test_bit(HFC_CFG_NTMODE, &hc->chan[dch->channel].cfg)) {
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: PH_ACTIVATE port %d (0..%d)\n", __FUNCTION__, hc->chan[dch->channel].port, hc->type-1);
			dch->inst.lock(hc, 0);
			/* start activation */
			if (hc->type == 1) {
				dchannel_sched_event(dch, D_L1STATECHANGE);
				if (debug & DEBUG_HFCMULTI_STATE)
					printk(KERN_ERR  "%s: E1 report state %x \n", __FUNCTION__, dch->ph_state);
			} else {
				HFC_outb(hc, R_ST_SEL, hc->chan[dch->channel].port);
			...
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: PH_ACTIVATE no NT-mode port %d (0..%d)\n", __FUNCTION__, hc->chan[dch->channel].port, hc->type-1);
			ret = -EINVAL;
		}
	} else 
	if (hh->prim == (PH_DEACTIVATE | REQUEST)) {
		if (test_bit(HFC_CFG_NTMODE, &hc->chan[dch->channel].cfg)) {
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: PH_DEACTIVATE port %d (0..%d)\n", __FUNCTION__, hc->chan[dch->channel].port, hc->type-1);
			dch->inst.lock(hc, 0);
			...
			if (hc->type == 1) {
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: PH_DEACTIVATE no BRI\n", __FUNCTION__);
			} else {
				HFC_outb(hc, R_ST_SEL, hc->chan[dch->channel].port);
			...
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: PH_DEACTIVATE no NT-mode port %d (0..%d)\n", __FUNCTION__, hc->chan[dch->channel].port, hc->type-1);
			ret = -EINVAL;
		}
	} else {
		if (debug & DEBUG_HFCMULTI_MSG)
			printk(KERN_ERR  "%s: unknown prim %x\n", __FUNCTION__, hh->prim);
		ret = -EINVAL;
	}
	if (!ret) {
//		printk("1\n");
 ...
 */
static int
hfcmulti_l2l1(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	u_long		num;
	int		slot_tx, slot_rx, bank_tx, bank_rx;
	bchannel_t	*bch;
	...
	if ((hh->prim == (PH_ACTIVATE | REQUEST))
	 || (hh->prim == (DL_ESTABLISH  | REQUEST))) {
		...
		if (debug & DEBUG_HFCMULTI_MSG)
			printk(KERN_ERR  "%s: PH_ACTIVATE ch %d (0..32)\n", __FUNCTION__, bch->channel);
		if (test_and_set_bit(BC_FLG_ACTIV, &bch->Flag))
			...
			if (!ret) {
				...
				if (bch->protocol==ISDN_PID_L1_B_64TRANS && !hc->dtmf) {
					...
					hc->dtmf = 1;
					if (debug & DEBUG_HFCMULTI_DTMF)
						printk(KERN_ERR  "%s: start dtmf decoder\n", __FUNCTION__);
	...
	if ((hh->prim == (PH_DEACTIVATE | REQUEST))
	 ...
	 || (hh->prim == (MGR_DISCONNECT | REQUEST))) {
		if (debug & DEBUG_HFCMULTI_MSG)
			printk(KERN_ERR  "%s: PH_DEACTIVATE ch %d (0..32)\n", __FUNCTION__, bch->channel);
...
//printk("7\n");
	...
	if (hh->prim == (PH_CONTROL | REQUEST)) {
		...
		switch (hh->dinfo) {
			...
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: HW_FEATURE request\n", __FUNCTION__);
			...
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: HW_CONF_JOIN conf %ld\n", __FUNCTION__, num);
			if (num <= 7) {
			...
			case HW_CONF_SPLIT:
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: HW_CONF_SPLIT\n", __FUNCTION__);
			...
			case HW_SPL_LOOP_ON:
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: HW_SPL_LOOP_ON (len = %d)\n", __FUNCTION__, skb->len);
			...
			if (debug & DEBUG_HFCMULTI_MSG)
				printk(KERN_ERR  "%s: HW_SPL_LOOP_OFF\n", __FUNCTION__);
	...
	if (test_and_clear_bit(D_L1STATECHANGE, &dch->event)) {
		if (hc->type == 1) {
			if (!test_bit(HFC_CFG_NTMODE, &hc->chan[ch].cfg)) {
				if (debug & DEBUG_HFCMULTI_STATE)
					printk(KERN_ERR  "%s: E1 TE newstate %x\n", __FUNCTION__, dch->ph_state);
			} else { 
				if (debug & DEBUG_HFCMULTI_STATE)
					printk(KERN_ERR  "%s: E1 NT newstate %x\n", __FUNCTION__, dch->ph_state);
			}
			switch (dch->ph_state) {
				case (1):
			...
			if (!test_bit(HFC_CFG_NTMODE, &hc->chan[ch].cfg)) {
				if (debug & DEBUG_HFCMULTI_STATE)
					printk(KERN_ERR  "%s: S/T TE newstate %x\n", __FUNCTION__, dch->ph_state);
				switch (dch->ph_state) {
					case (0):
				...
				if (debug & DEBUG_HFCMULTI_STATE)
					printk(KERN_ERR  "%s: S/T NT newstate %x\n", __FUNCTION__, dch->ph_state);
				dch->inst.lock(hc, 0);
				switch (dch->ph_state) {
					case (2):
...
hfcmulti_initmode(hfc_multi_t *hc)
{
	...
	if (hc->type != 1) {
		...
		if (test_bit(HFC_CHIP_PCM_SLAVE, &hc->chip)) {
			/* SLAVE (clock master) */
			if (debug & DEBUG_HFCMULTI_INIT)
				printk(KERN_ERR  "%s: E1 port is clock master\n", __FUNCTION__);
//			HFC_outb(hc, R_SYNC_CTRL, V_SYNC_OFFS | V_PCM_SYNC);
			HFC_outb(hc, R_SYNC_CTRL, V_EXT_CLK_SYNC | V_PCM_SYNC);
		} else {
			/* MASTER (clock slave) */
			if (debug & DEBUG_HFCMULTI_INIT)
				printk(KERN_ERR  "%s: E1 port is clock slave\n", __FUNCTION__);
			HFC_outb(hc, R_SYNC_CTRL, V_SYNC_OFFS);
		}
		if (test_bit(HFC_CFG_NTMODE, &hc->chan[(i<<2)+2].cfg)) {
			if (debug & DEBUG_HFCMULTI_INIT)
				printk(KERN_ERR  "%s: E1 port is NT-mode\n", __FUNCTION__);
			r_e1_wr_sta = 0; /* G0 */
		}else {
			if (debug & DEBUG_HFCMULTI_INIT)
				printk(KERN_ERR  "%s: E1 port is TE-mode\n", __FUNCTION__);
			r_e1_wr_sta = 0; /* F0 */
		}
...
init_card(hfc_multi_t *hc)
{
	...
	if (request_irq(hc->pci_dev->irq, hfcmulti_interrupt, SA_SHIRQ, "HFC-multi", hc)) {
		printk(KERN_WARNING "AS_ISDN: Could not get interrupt %d.\n", hc->pci_dev->irq);
		unlock_dev(hc);
		return(-EIO);
	}
	...
	if (debug & DEBUG_HFCMULTI_INIT)
		printk(KERN_ERR  "%s: IRQ %d count %d\n", __FUNCTION__, hc->irq, hc->irqcnt);
	while (cnt) {
		if ((err = init_chip(hc)))
			goto error;
...
setup_pci(hfc_multi_t *hc)
{
	...
	while (id_list[i].vendor_id) {
		...
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "setup_pci(): investigating card entry %d (looking for type %d)\n", i, hc->type);
		inuse:
...
//	if (debug & DEBUG_HFCMULTI_INIT)
//		printk(KERN_ERR  "%s: before lock_dev\n", __FUNCTION__);
	lock_dev(hc, 0);
//	if (debug & DEBUG_HFCMULTI_INIT)
//		printk(KERN_ERR  "%s: after lock_dev\n", __FUNCTION__);
	...
	while(i < 32) {
		...
		if (hc->chan[i].port==port || all) {
			if (hc->chan[i].dch) {
				if (debug & DEBUG_HFCMULTI_INIT)
					printk(KERN_ERR  "%s: free port %d D-channel %d (1..32)\n", __FUNCTION__, hc->chan[i].port, i);
			...
			if (hc->chan[i].bch) {
				if (debug & DEBUG_HFCMULTI_INIT)
					printk(KERN_ERR  "%s: free port %d B-channel %d (1..32)\n", __FUNCTION__, hc->chan[i].port, i);
	...
	if (all) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: do release_io_hfcmulti\n", __FUNCTION__);
...
//printk(KERN_ERR  "comparing (D-channel) card=%08x inst=%08x with inst=%08x\n", hc, &hc->dch[i].inst, inst);
	...
	switch(prim) {
		...
		if (dch)
			return(AS_ISDN_SetIF(inst, arg, prim, hfcmulti_l1hw, NULL, dch));
		if (bch)
			return(AS_ISDN_SetIF(inst, arg, prim, hfcmulti_l2l1, NULL, bch));
		//break;
...
HFCmulti_init(void)
{
	int err, err2, i;
	hfc_multi_t *hc,*next;
	AS_ISDN_pid_t pid, pids[MAX_CARDS];
	...
	if (debug & DEBUG_HFCMULTI_INIT)
		printk(KERN_ERR  "%s: init entered\n", __FUNCTION__);

#ifdef __BIG_ENDIAN
#error "not running on big endian machines now"
#endif
	strcpy(tmp, hfcmulti_revision);
	printk(KERN_INFO "AS_ISDN: HFC-multi driver Rev. %s\n", AS_ISDN_getrev(tmp));
	...
	if (debug & DEBUG_HFCMULTI_INIT)
		printk(KERN_ERR  "%s: registering HFCM_obj\n", __FUNCTION__);
	if ((err = as_isdn_register(&HFCM_obj))) {
		printk(KERN_ERR "Can't register HFC-Multi cards error(%d)\n", err);
		return(err);
	}
	if (debug & DEBUG_HFCMULTI_INIT)
		printk(KERN_ERR  "%s: new AS_ISDN object (refcnt = %d)\n", __FUNCTION__, HFCM_obj.refcnt);
	...
	while (HFC_cnt < MAX_CARDS  &&  type[HFC_cnt] > 0) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: Registering chip type %d (0x%x)\n", __FUNCTION__, type[HFC_cnt] & 0xff, type[HFC_cnt]);
...
//			test_and_set_bit(HFC_CHIP_LEDS, &hc->chip);
		...
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: (after APPEND_TO_LIST)\n", __FUNCTION__);
		...
		while (pt < hc->type) {
			...
			if (hc->type == 1)
				...
				ch = (pt<<2)+2;
			if (debug & DEBUG_HFCMULTI_INIT)
				printk(KERN_ERR  "%s: Registering D-channel, card(%d) ch(%d) port(%d) protocol(%x)\n", __FUNCTION__, HFC_cnt+1, ch, pt+1, protocol[port_cnt]);
			hc->chan[ch].port = pt;
			hc->chan[ch].nt_timer = -1;
			...
			AS_ISDN_init_instance(&dch->inst, &HFCM_obj, hc);
			...
			while(i < bchperport) {
				if (hc->type == 1)
					...
					ch2 = (pt<<2)+i;
				if (debug & DEBUG_HFCMULTI_INIT)
					printk(KERN_ERR  "%s: Registering B-channel, card(%d) ch(%d) port(%d) channel(%d)\n", __FUNCTION__, HFC_cnt+1, ch2, pt+1, i);
				hc->chan[ch2].port = pt;
				...
				bch->channel = ch2;
				AS_ISDN_init_instance(&bch->inst, &HFCM_obj, hc);
			...
			/* set D-channel */
			AS_ISDN_set_dchannel_pid(&pid, protocol[port_cnt], layermask[port_cnt]);
			...
			if (hc->type != 1) {
				...
				if (protocol[port_cnt] & 0x10000) {
					if (debug & DEBUG_HFCMULTI_INIT)
						printk(KERN_ERR  "%s: PROTOCOL set optical interfacs: card(%d) port(%d)\n", __FUNCTION__, HFC_cnt+1, pt);
				...
				if (protocol[port_cnt] & 0x40000) {
					if (debug & DEBUG_HFCMULTI_INIT)
						printk(KERN_ERR  "%s: PROTOCOL set LOS report: card(%d) port(%d)\n", __FUNCTION__, HFC_cnt+1, pt);
				...
				if (protocol[port_cnt] & 0x80000) {
					if (debug & DEBUG_HFCMULTI_INIT)
						printk(KERN_ERR  "%s: PROTOCOL set AIS report: card(%d) port(%d)\n", __FUNCTION__, HFC_cnt+1, pt);
				...
				if (protocol[port_cnt] & 0x100000) {
					if (debug & DEBUG_HFCMULTI_INIT)
						printk(KERN_ERR  "%s: PROTOCOL set SLIP report: card(%d) port(%d)\n", __FUNCTION__, HFC_cnt+1, pt);
				...
				if (protocol[port_cnt] & 0x600000) {
					if (debug & DEBUG_HFCMULTI_INIT)
						printk(KERN_ERR  "%s: PROTOCOL set elastic buffer to %d: card(%d) port(%d)\n", __FUNCTION__, hc->chan[ch].jitter, HFC_cnt+1, pt);
		...
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: Initializing card(%d)\n", __FUNCTION__, HFC_cnt+1);
		if ((err = init_card(hc))) {
			if (debug & DEBUG_HFCMULTI_INIT) {
				printk(KERN_ERR  "%s: do release_io_hfcmulti\n", __FUNCTION__);
				release_io_hfcmulti(hc);
		...
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_ERR  "%s: Init modes card(%d)\n", __FUNCTION__, HFC_cnt+1);
		...
		while(pt < hc->type) {
			if (debug & DEBUG_HFCMULTI_INIT)
				printk(KERN_ERR  "%s: Adding d-stack: card(%d) port(%d)\n", __FUNCTION__, HFC_cnt+1, pt+1);
			if (hc->type == 1)
				dch = hc->chan[16].dch;
			else
			...
			while(i < bchperport) {
				if (debug & DEBUG_HFCMULTI_INIT)
					printk(KERN_ERR  "%s: Adding b-stack: card(%d) port(%d) B-channel(%d)\n", __FUNCTION__, HFC_cnt+1, pt+1, i+1);
				if (hc->type == 1)
			...
			if (debug & DEBUG_HFCMULTI_INIT)
				printk(KERN_ERR  "%s: (after MGR_SETSTACK REQUEST)\n", __FUNCTION__);
	...
	free_object:
	if (debug & DEBUG_HFCMULTI_INIT)
		printk(KERN_ERR  "%s: brefore as_isdn_unregister (refcnt = %d)\n", __FUNCTION__, HFCM_obj.refcnt);
	if ((err2 = as_isdn_unregister(&HFCM_obj))) {
	...
	if (debug & DEBUG_HFCMULTI_INIT)
		printk(KERN_ERR  "%s: after as_isdn_unregister (refcnt = %d)\n", __FUNCTION__, HFCM_obj.refcnt);
...
HFCmulti_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&HFCM_obj))) {
		printk(KERN_ERR "Can't unregister HFC-Multi cards error(%d)\n", err);
	}
	if (debug & DEBUG_HFCMULTI_INIT)
