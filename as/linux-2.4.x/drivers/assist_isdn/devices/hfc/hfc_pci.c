/*
 * $Id: hfc_pci.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
...
#define HFC_INFO(txt)	printk(KERN_ERR  txt)
...
{
	register AS_ISDN_HWlock_t	*lock = &((hfc_pci_t *)data)->lock;
...
reset_hfcpci(hfc_pci_t *hc)
{
	...
	printk(KERN_ERR  "HFC-PCI status(%x) before reset\n", val);
	...
	Write_hfc(hc, HFCPCI_CIRM, hc->hw.cirm);
	val = Read_hfc(hc, HFCPCI_STATUS);
	printk(KERN_ERR  "HFC-PCI status(%x) after reset\n", val);
	...
	printk(KERN_ERR  "HFC-PCI status(%x) after %dus\n", val, cnt);
...
{       u_char fifo_state;
	...
	if (hc->bch[fifo].debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&hc->bch[fifo].inst, "hfcpci_clear_fifo_tx%d f1(%x) f2(%x) z1(%x) z2(%x) state(%x)",
	...
	Write_hfc(hc, HFCPCI_FIFO_EN, hc->hw.fifo_en);
	if (hc->bch[fifo].debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&hc->bch[fifo].inst, "hfcpci_clear_fifo_tx%d f1(%x) f2(%x) z1(%x) z2(%x)",
...
hfcpci_empty_fifo(bchannel_t *bch, bzfifo_type * bz, u_char * bdata, int count)
{
	...
	if ((bch->debug & L1_DEB_HSCX) && !(bch->debug & L1_DEB_HSCX_FIFO))
		AS_ISDN_debugprint(&bch->inst, "hfcpci_empty_fifo");
	...
	if ((count > MAX_DATA_SIZE + 3) || (count < 4) ||
		...
		if (bch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&bch->inst, "hfcpci_empty_fifo: incoming packet invalid length %d or crc", count);
...
receive_dmsg(hfc_pci_t *hc)
{
	...
	while (((df->f1 & D_FREG_MASK) != (df->f2 & D_FREG_MASK)) && count--) {
		...
		if (dch->debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&dch->inst, "hfcpci recd f1(%d) f2(%d) z1(%x) z2(%x) cnt(%d)",
				df->f1, df->f2, zp->z1, zp->z2, rcnt);

		if ((rcnt > MAX_DFRAME_LEN + 3) || (rcnt < 4) ||
		    (df->data[zp->z1])) {
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "empty_fifo hfcpci paket inv. len %d or crc %d", rcnt, df->data[zp->z1]);
#ifdef ERROR_STATISTIC
			cs->err_rx++;
#endif
			...
			if (dch->debug & L1_DEB_ISAC_FIFO) {
				...
				AS_ISDN_QuickHex(t, ptr, count);
				AS_ISDN_debugprint(&dch->inst, dch->dlog);
			}
...
main_rec_hfcpci(bchannel_t *bch)
{
	...
	if (bz->f1 != bz->f2) {
		if (bch->debug & L1_DEB_HSCX)
			AS_ISDN_debugprint(&bch->inst, "hfcpci rec ch(%x) f1(%d) f2(%d)",
		...
		rcnt++;
		if (bch->debug & L1_DEB_HSCX)
			AS_ISDN_debugprint(&bch->inst, "hfcpci rec ch(%x) z1(%x) z2(%x) cnt(%d)",
...
hfcpci_fill_dfifo(hfc_pci_t *hc)
{
	...
	if ((dch->debug & L1_DEB_ISAC) && !(dch->debug & L1_DEB_ISAC_FIFO))
		AS_ISDN_debugprint(&dch->inst, "hfcpci_fill_dfifo");
	...
	if (dch->debug & L1_DEB_ISAC_FIFO)
		AS_ISDN_debugprint(&dch->inst, "hfcpci_fill_Dfifo f1(%d) f2(%d) z1(f1)(%x)",
			df->f1, df->f2,
	...
	if (fcnt > (MAX_D_FRAMES - 1)) {
		if (dch->debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&dch->inst, "hfcpci_fill_Dfifo more as 14 frames");
#ifdef ERROR_STATISTIC
		cs->err_tx++;
#endif
		return;
	...
	if (dch->debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&dch->inst, "hfcpci_fill_Dfifo count(%ld/%d)",
			count, maxlen);
	if (count > maxlen) {
		if (dch->debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&dch->inst, "hfcpci_fill_Dfifo no fifo mem");
		return;
	}
...
hfcpci_fill_fifo(bchannel_t *bch)
{
	...
	if (bch->protocol == ISDN_PID_L1_B_64TRANS) {
		...
		z2t = z1t + 1;
		if (bch->debug & L1_DEB_HSCX)
			AS_ISDN_debugprint(&bch->inst, "hfcpci_fill_fifo_trans ch(%x) cnt(%d) z1(%x) z2(%x)",
...
next_t_frame:
		if (fcnt < (2 * HFCPCI_BTRANS_THRESHOLD)) {
			...
			if (count < B_FIFO_SIZE - fcnt) {
				...
				if (bch->debug & L1_DEB_HSCX_FIFO)
					AS_ISDN_debugprint(&bch->inst, "hfcpci_FFt fcnt(%d) maxl(%d) nz1(%x) dst(%p)",
				...
				*z1t = new_z1;	/* now send data */
				...
				AS_ISDN_debugprint(&bch->inst, "hfcpci_fill_fifo_trans ch(%x) frame length %d discarded",
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "%s: ch(%x) f1(%d) f2(%d) z1(f1)(%x)",
	...
	if (fcnt > (MAX_B_FRAMES - 1)) {
		if (bch->debug & L1_DEB_HSCX)
			AS_ISDN_debugprint(&bch->inst, "hfcpci_fill_Bfifo more as 14 frames");
		return;
	}
	/* now determine free bytes in FIFO buffer */
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "hfcpci_fill_fifo ch(%x) count(%ld/%d),%lx",
			bch->channel, count,
			maxlen, current->state);

	if (maxlen < count) {
		if (bch->debug & L1_DEB_HSCX)
			AS_ISDN_debugprint(&bch->inst, "hfcpci_fill_fifo no fifo mem");
		return;
	}
		...
		if ((rcnt > 256 + 3) || (count < 4) ||
			...
			if (hc->debug & DEB_DLOG_HEX) {
				...
				if ((total - 3) < MAX_DLOG_SPACE / 3 - 10) {
					...
					*ptr++ = 'O';
					*ptr++ = ':';
					ptr += AS_ISDN_QuickHex(ptr, e_buffer, total - 3);
					ptr--;
					*ptr++ = '\n';
					*ptr = 0;
					AS_ISDN_putstatus(hc, NULL, cs->dlog);
				} else
					AS_ISDN_putstatus(hc, "LogEcho: ", "warning Frame too big (%d)", total - 3);
			}
		}
...
hfcpci_interrupt(int intno, void *dev_id, struct pt_regs *regs)
{
	...
	if (HFCPCI_ANYINT & (stat = Read_hfc(hc, HFCPCI_STATUS))) {
		...
		if (hc->dch.debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&hc->dch.inst, "HFC-PCI: stat(%02x) s1(%02x)",
				stat, val);
	} else {
		/* shared */
#ifdef SPIN_DEBUG
	...
	if (hc->dch.debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&hc->dch.inst, "HFC-PCI irq %x", val);
	val &= hc->hw.int_m1;
	if (val & 0x40) {	/* state machine irq */
		exval = Read_hfc(hc, HFCPCI_STATES) & 0xf;
		if (hc->dch.debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&hc->dch.inst, "ph_state chg %d->%d",
	...
	if (val & 0x08) {
		if (!(bch = Sel_BCS(hc, hc->hw.bswapped ? 2 : 1))) {
			if (hc->dch.debug)
				AS_ISDN_debugprint(&hc->dch.inst, "hfcpci spurious 0x08 IRQ");
		} else
			main_rec_hfcpci(bch);
	}
	if (val & 0x10) {
...
//		else 
		if (!(bch = Sel_BCS(hc, 2))) {
			if (hc->dch.debug)
				AS_ISDN_debugprint(&hc->dch.inst, "hfcpci spurious 0x10 IRQ");
		} else
			main_rec_hfcpci(bch);
	}
	if (val & 0x01) {
		if (!(bch = Sel_BCS(hc, hc->hw.bswapped ? 2 : 1))) {
			if (hc->dch.debug)
				AS_ISDN_debugprint(&hc->dch.inst, "hfcpci spurious 0x01 IRQ");
		} else {
	...
	if (val & 0x02) {
		if (!(bch = Sel_BCS(hc, 2))) {
			if (hc->dch.debug)
				AS_ISDN_debugprint(&hc->dch.inst, "hfcpci spurious 0x02 IRQ");
		} else {
			if (bch->tx_idx < bch->tx_len) {
				hfcpci_fill_fifo(bch);
			} else {
...
/*************************************/
static int
HFCD_l1hw(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	dchannel_t	*dch;
	hfc_pci_t	*hc;
	int		ret = -EINVAL;
...
//			l1_msg(hc, HW_POWERUP | CONFIRM, NULL);
			...
			if (1 & hh->dinfo) {
				if (test_bit(HFC_CFG_SW_DD_DU, &hc->cfg))
					...
					slot = 0x80;
				printk(KERN_ERR   "%s: Write_hfc: B1_SSL/RSL 0x%x\n",
					__FUNCTION__, slot);
			...
			if (2 & hh->dinfo) {
				if (test_bit(HFC_CFG_SW_DD_DU, &hc->cfg))
					...
					slot = 0x81;
				printk(KERN_ERR  "%s: Write_hfc: B2_SSL/RSL 0x%x\n",
					__FUNCTION__, slot);
			...
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "%s: unknown ctrl %x",
		...
		if (hc->hw.nt_mode) {
			...
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "%s: PH_ACTIVATE none NT mode",
					__FUNCTION__);
			ret = -EINVAL;
		}
		...
		if (hc->hw.nt_mode) {
			...
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "%s: PH_DEACTIVATE none NT mode",
					__FUNCTION__);
			ret = -EINVAL;
		}
	} else {
		if (dch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&dch->inst, "%s: unknown prim %x",
...
mode_hfcpci(bchannel_t *bch, int bc, int protocol)
{
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "HFCPCI bchannel protocol %x-->%x ch %x-->%x",
...
#endif
		default:
			AS_ISDN_debugprint(&bch->inst, "prot not known %x", protocol);
			return(-ENOPROTOOPT);
	}
	if (test_bit(HFC_CFG_PCM, &hc->cfg)) {
		if ((protocol == ISDN_PID_NONE) ||
		...
		if (bc & 2) {
			hc->hw.conn &= 0xc7;
			hc->hw.conn |= 0x08;
			printk(KERN_ERR  "%s: Write_hfc: B2_SSL 0x%x\n",
				__FUNCTION__, tx_slot);
			printk(KERN_ERR  "%s: Write_hfc: B2_RSL 0x%x\n",
			...
			hc->hw.conn &= 0xf8;
			hc->hw.conn |= 0x01;
			printk(KERN_ERR  "%s: Write_hfc: B1_SSL 0x%x\n",
				__FUNCTION__, tx_slot);
			printk(KERN_ERR  "%s: Write_hfc: B1_RSL 0x%x\n",
				__FUNCTION__, rx_slot);
...
set_hfcpci_rxtest(bchannel_t *bch, int protocol, struct sk_buff *skb)
{
	...
	if (skb->len <4) {
		AS_ISDN_debugprint(&bch->inst, "HFCPCI rxtest no channel parameter");
		return(-EINVAL);
	}
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "HFCPCI bchannel test rx protocol %x-->%x ch %x-->%x",
			bch->protocol, protocol, bch->channel, *chan);
	if (bch->channel != *chan) {
		AS_ISDN_debugprint(&bch->inst, "HFCPCI rxtest wrong channel parameter %x/%x",
			bch->channel, *chan);
		return(-EINVAL);
	}
	switch (protocol) {
...
/******************************/
static int
hfcpci_l2l1(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	bchannel_t	*bch;
	int		ret = -EINVAL;
	AS_ISDN_head_t	*hh;
...
HW_hfcD_bh(dchannel_t *dch)
{
	...
	if (test_and_clear_bit(D_L1STATECHANGE, &dch->event)) {
		if (!hc->hw.nt_mode) {
			if (dch->debug)
				printk(KERN_ERR  "%s: TE newstate %x\n",
			...
			if (dch->debug)
				printk(KERN_ERR  "%s: NT newstate %x\n",
					__FUNCTION__, dch->ph_state);
			dch->inst.lock(dch->inst.data, 0);
			switch (dch->ph_state) {
				case (2):
...
static int init_card(hfc_pci_t *hc)
{
	...
	if (request_irq(hc->irq, hfcpci_interrupt, SA_SHIRQ, "HFC PCI", hc)) {
		printk(KERN_WARNING "AS_ISDN: couldn't get interrupt %d\n",
			hc->irq);
		unlock_dev(hc);
		return(-EIO);
	}
...
static AS_ISDNobject_t	HFC_obj;
static int debug;
...
#error "not running on big endian machines now"
	...
	printk(KERN_INFO "AS_ISDN: HFC-PCI driver Rev. %s\n", AS_ISDN_getrev(tmp));
	hc->hw.cirm = 0;
	hc->dch.ph_state = 0;
	...
	if (tmp_hfcpci) {
		...
		printk(KERN_INFO "AS_ISDN: HFC-PCI card manufacturer: %s card name: %s\n", id_list[i].vendor_name, id_list[i].card_name);
	} else {
...
release_card(hfc_pci_t *hc) {
	...
	release_io_hfcpci(hc);
	AS_ISDN_free_bch(&hc->bch[1]);
	AS_ISDN_free_bch(&hc->bch[0]);
	AS_ISDN_free_dch(&hc->dch);
...
HFC_manager(void *data, u_int prim, void *arg) {
	...
	switch(prim) {
		...
		if (channel==2)
			return(AS_ISDN_SetIF(inst, arg, prim, HFCD_l1hw, NULL,
				&card->dch));
		else
			return(AS_ISDN_SetIF(inst, arg, prim, hfcpci_l2l1, NULL,
				&card->bch[channel]));
...
static int __init HFC_init(void)
{
	...
	if ((err = as_isdn_register(&HFC_obj))) {
	...
	while (HFC_cnt < MAX_CARDS) {
		...
		AS_ISDN_init_instance(&card->dch.inst, &HFC_obj, card);
		card->dch.inst.pid.layermask = ISDN_LAYER(0);
		sprintf(card->dch.inst.name, "HFC%d", HFC_cnt+1);
		AS_ISDN_init_dch(&card->dch);
		for (i=0; i<2; i++) {
			card->bch[i].channel = i + 1;
			AS_ISDN_init_instance(&card->bch[i].inst, &HFC_obj, card);
		...
		AS_ISDN_set_dchannel_pid(&pid, protocol[i], layermask[i]);
		...
		printk(KERN_ERR  "HFC card %p dch %p bch1 %p bch2 %p\n",
			card, &card->dch, &card->bch[0], &card->bch[1]);
		if (setup_hfcpci(card)) {
			err = 0;
			AS_ISDN_free_dch(&card->dch);
			AS_ISDN_free_bch(&card->bch[1]);
			AS_ISDN_free_bch(&card->bch[0]);
			list_del(&card->list);
			kfree(card);
			if (!HFC_cnt) {
				as_isdn_unregister(&HFC_obj);
...
static void __exit HFC_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&HFC_obj))) {
