/*
 * $Id: hfcs_usb.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
 * AS_ISDN driver for Colognechip HFC-S USB chip
 * TODO
 *   - hotplug disconnect the USB TA does not unregister AS_ISDN Controller
...
#define DRIVER_NAME "AS_ISDN_hfcsusb"
...
static AS_ISDNobject_t hw_AS_ISDNObj;
...
static int hfcsusb_l1hwD(AS_ISDNif_t * hif, struct sk_buff *skb);
static int hfcsusb_l2l1B(AS_ISDNif_t * hif, struct sk_buff *skb);
...
{
	register AS_ISDN_HWlock_t *lock = &((hfcsusb_t *) data)->lock;
...
hfcsusb_manager(void *data, u_int prim, void *arg)
{
	...
	if (!data) {
		MGR_HASPROTOCOL_HANDLER(prim, arg, &hw_AS_ISDNObj)
		    printk(KERN_ERR "%s: no data prim %x arg %p\n",
			   __FUNCTION__, prim, arg);
		return (-EINVAL);
	}
	list_for_each_entry(card, &hw_AS_ISDNObj.ilist, list) {
	...
	switch (prim) {
		...
		case MGR_UNREGLAYER | REQUEST:
			...
			hw_AS_ISDNObj.ctrl(inst->up.peer,
					 MGR_DISCONNECT | REQUEST,
					 &inst->up);
			hw_AS_ISDNObj.ctrl(inst, MGR_UNREGLAYER | REQUEST,
					 NULL);
			break;
...
S0_new_state(dchannel_t * dch)
{
	...
	if (card->hw_mode & HW_MODE_TE) {
		if (dch->debug)
			AS_ISDN_debugprint(&card->dch.inst,
				 "%s: TE %d",
...
mode_bchannel(bchannel_t * bch, int bc, int protocol)
{
	...
	switch (protocol) {
		...
		case (ISDN_PID_L1_B_64HDLC):
			...
			break;
		default:
			AS_ISDN_debugprint(&bch->inst, "prot not known %x",
					 protocol);
			return (-ENOPROTOOPT);
	}
...
hfcsusb_ph_command(hfcsusb_t * card, u_char command)
{
	if (card->dch.debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&card->dch.inst, "hfcsusb_ph_command %x",
				 command);
...
/*************************************/
static int
hfcsusb_l1hwD(AS_ISDNif_t * hif, struct sk_buff *skb)
{
	dchannel_t *dch;;
	int ret = -EINVAL;
	AS_ISDN_head_t *hh;
...
/******************************/
static int
hfcsusb_l2l1B(AS_ISDNif_t * hif, struct sk_buff *skb)
{
	bchannel_t *bch;
	int ret = -EINVAL;
	AS_ISDN_head_t *hh;
...
release_card(hfcsusb_t * card)
{
	...
	hw_AS_ISDNObj.ctrl(card->dch.inst.up.peer, MGR_DISCONNECT | REQUEST,
			 &card->dch.inst.up);
	hw_AS_ISDNObj.ctrl(&card->dch.inst, MGR_UNREGLAYER | REQUEST, NULL);
	list_del(&card->list);
	unlock_dev(card);
...
setup_instance(hfcsusb_t * card)
{
	...
	list_add_tail(&card->list, &hw_AS_ISDNObj.ilist);
	...
	AS_ISDN_init_instance(&card->dch.inst, &hw_AS_ISDNObj, card);
	sprintf(card->dch.inst.name, "hfcsusb_%d", hfcsusb_cnt + 1);
	AS_ISDN_set_dchannel_pid(&pid, protocol[hfcsusb_cnt],
	...
	for (i = 0; i < 2; i++) {
		...
		AS_ISDN_init_instance(&card->bch[i].inst, &hw_AS_ISDNObj,
				    card);
	...
	hfcsusb_cnt++;
	err =
	    hw_AS_ISDNObj.ctrl(NULL, MGR_NEWSTACK | REQUEST,
			     &card->dch.inst);
	if (err) {
		release_card(card);
		return (err);
	...
	for (i = 0; i < 2; i++) {
		err =
		    hw_AS_ISDNObj.ctrl(card->dch.inst.st,
	...
	err =
	    hw_AS_ISDNObj.ctrl(card->dch.inst.st, MGR_SETSTACK | REQUEST,
			     &pid);
	if (err) {
		printk(KERN_ERR "MGR_SETSTACK REQUEST dch err(%d)\n", err);
		hw_AS_ISDNObj.ctrl(card->dch.inst.st, MGR_DELSTACK | REQUEST,
				 NULL);
		return (err);
	}

	hw_init(card);
	hw_AS_ISDNObj.ctrl(card->dch.inst.st, MGR_CTRLREADY | INDICATION,
			 NULL);
...
hfcsusb_init(void)
{
	...
	printk(KERN_INFO "hfcsusb driver Rev. %s (debug=%i)\n",
	       AS_ISDN_getrev(hfcsusb_rev), debug);

#ifdef MODULE
	hw_AS_ISDNObj.owner = THIS_MODULE;
#endif
	INIT_LIST_HEAD(&hw_AS_ISDNObj.ilist);
	hw_AS_ISDNObj.name = DRIVER_NAME;
	hw_AS_ISDNObj.own_ctrl = hfcsusb_manager;
	
	hw_AS_ISDNObj.DPROTO.protocol[0] = ISDN_PID_L0_TE_S0 |
					 ISDN_PID_L0_NT_S0;
	hw_AS_ISDNObj.DPROTO.protocol[1] = ISDN_PID_L1_NT_S0;
	hw_AS_ISDNObj.BPROTO.protocol[1] = ISDN_PID_L1_B_64TRANS |
					 ISDN_PID_L1_B_64HDLC;
	hw_AS_ISDNObj.BPROTO.protocol[2] = ISDN_PID_L2_B_TRANS |
					 ISDN_PID_L2_B_RAWDEV;
				     	
	if ((err = as_isdn_register(&hw_AS_ISDNObj))) {
	...
	return 0;
      ...
      out:
	as_isdn_unregister(&hw_AS_ISDNObj);
	return err;
}
...
hfcsusb_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&hw_AS_ISDNObj))) {
		printk(KERN_ERR "Can't unregister hfcsusb error(%d)\n",
		       err);
	}

	list_for_each_entry_safe(card, next, &hw_AS_ISDNObj.ilist, list) {
