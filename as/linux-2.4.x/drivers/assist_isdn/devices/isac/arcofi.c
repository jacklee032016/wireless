/*
 * $Id: arcofi.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
...
arcofi_fsm(dchannel_t *dch, int event, void *data) {
	...
	if (dch->debug & L1_DEB_MONITOR) {
		AS_ISDN_debugprint(&dch->inst, "arcofi state %d event %d", isac->arcofi_state, event);
	}
	...
	switch (isac->arcofi_state) {
		...
		case ARCOFI_RECEIVE:
			...
			break;
		default:
			AS_ISDN_debugprint(&dch->inst, "Arcofi unknown state %x", isac->arcofi_state);
			return(2);
	}
	return(0);
}
