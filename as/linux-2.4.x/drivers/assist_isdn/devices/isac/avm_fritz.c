/*
 * $Id: avm_fritz.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
...
{
	register AS_ISDN_HWlock_t	*lock = &((fritzpnppci *)data)->lock;
...
write_ctrl(bchannel_t *bch, int which) {
	...
	if (fc->dch.debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "hdlc %c wr%x ctrl %x",
...
modehdlc(bchannel_t *bch, int bc, int protocol)
{
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "hdlc %c protocol %x-->%x ch %d-->%d",
...
hdlc_empty_fifo(bchannel_t *bch, int count)
{
	...
	if ((fc->dch.debug & L1_DEB_HSCX) && !(fc->dch.debug & L1_DEB_HSCX_FIFO))
		AS_ISDN_debugprint(&bch->inst, "hdlc_empty_fifo %d", count);
	if (bch->rx_idx + count > MAX_DATA_MEM) {
		if (fc->dch.debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&bch->inst, "hdlc_empty_fifo: incoming packet too large");
		return;
	}
...
HDLC_irq(bchannel_t *bch, u_int stat)
{
	...
	if (stat & HDLC_INT_RPR) {
		if (stat & HDLC_STAT_RDO) {
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "RDO");
			else
				AS_ISDN_debugprint(&bch->inst, "ch%d stat %#x", bch->channel, stat);
			hdlc->ctrl.sr.xml = 0;
			...
			if ((stat & HDLC_STAT_RME) || (bch->protocol == ISDN_PID_L1_B_64TRANS)) {
				if (((stat & HDLC_STAT_CRCVFRRAB)==HDLC_STAT_CRCVFR) ||
					...
					if (bch->debug & L1_DEB_HSCX)
						AS_ISDN_debugprint(&bch->inst, "invalid frame");
					else
						AS_ISDN_debugprint(&bch->inst, "ch%d invalid frame %#x", bch->channel, stat);
					bch->rx_idx = 0;
				}
			}
		}
	...
	if (stat & HDLC_INT_XDU) {
		...
		if (bch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&bch->inst, "ch%d XDU tx_len(%d) tx_idx(%d) Flag(%lx)",
...
HDLC_irq_main(fritzpnppci *fc)
{
	...
	if (stat & HDLC_INT_MASK) {
		if (!(bch = Sel_BCS(fc, 0))) {
			if (fc->bch[0].debug)
				AS_ISDN_debugprint(&fc->bch[0].inst, "hdlc spurious channel 0 IRQ");
		} else
			HDLC_irq(bch, stat);
	}
	...
	if (stat & HDLC_INT_MASK) {
		if (!(bch = Sel_BCS(fc, 1))) {
			if (fc->bch[1].debug)
				AS_ISDN_debugprint(&fc->bch[1].inst, "hdlc spurious channel 1 IRQ");
		} else
			HDLC_irq(bch, stat);
	}
}
...
avm_fritz_interrupt(int intno, void *dev_id, struct pt_regs *regs)
{
	...
	if (fc->dch.debug & L1_DEB_INTSTAT)
		AS_ISDN_debugprint(&fc->dch.inst, "irq stat0 %x", sval);
	...
	if (!(sval & AVM_STATUS0_IRQ_ISAC)) {
		...
		AS_ISDN_isac_interrupt(&fc->dch, val);
	}
...
avm_fritzv2_interrupt(int intno, void *dev_id, struct pt_regs *regs)
{
	...
	if (sval & AVM_STATUS0_IRQ_TIMER) {
		if (fc->dch.debug & L1_DEB_INTSTAT)
			AS_ISDN_debugprint(&fc->dch.inst, "Fc2 timer irq");
...
static int
hdlc_down(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	bchannel_t	*bch;
	int		ret = -EINVAL;
	AS_ISDN_head_t	*hh;
...
clear_pending_hdlc_ints(fritzpnppci *fc)
{
	...
	AS_ISDN_debugprint(&fc->dch.inst, "HDLC 1 STA %x", val);
	val = read_status(fc, 1);
	AS_ISDN_debugprint(&fc->dch.inst, "HDLC 2 STA %x", val);
}
...
static int init_card(fritzpnppci *fc)
{
	...
	while (cnt) {
		int	ret;
		AS_ISDN_clear_isac(&fc->dch);
		if ((ret=AS_ISDN_isac_init(&fc->dch))) {
			printk(KERN_WARNING "AS_ISDN: AS_ISDN_isac_init failed with %d\n", ret);
			break;
		}
...
setup_fritz(fritzpnppci *fc)
{
	...
	printk(KERN_INFO "AS_ISDN: %s config irq:%d base:0x%X\n",
	...
	switch(prim) {
		...
		if (channel == 2) {
			...
			if ((skb = create_link_skb(PH_CONTROL | REQUEST,
				...
				if (AS_ISDN_ISAC_l1hw(&inst->down, skb))
					dev_kfree_skb(skb);
			}
		} else {
		...
		if (channel==2)
			return(AS_ISDN_SetIF(inst, arg, prim, AS_ISDN_ISAC_l1hw, NULL,
				&card->dch));
		else
			return(AS_ISDN_SetIF(inst, arg, prim, hdlc_down, NULL,
				&card->bch[channel]));
		break;
...
static int __devinit setup_instance(fritzpnppci *card)
{
	...
	AS_ISDN_init_instance(&card->dch.inst, &fritz, card);
	sprintf(card->dch.inst.name, "Fritz%d", fritz_cnt+1);
	AS_ISDN_set_dchannel_pid(&pid, protocol[fritz_cnt], layermask[fritz_cnt]);
	AS_ISDN_init_dch(&card->dch);
	for (i=0; i<2; i++) {
		card->bch[i].channel = i;
		AS_ISDN_init_instance(&card->bch[i].inst, &fritz, card);
...
static int __devinit fritzpci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	printk(KERN_INFO "AS_ISDN_fcpcipnp: found adapter %s at %s\n",
...
#endif
{
	...
	printk(KERN_INFO "AS_ISDN_fcpcipnp: found adapter %s at IO %#x irq %d\n",
...
static void __exit Fritz_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&fritz))) {
