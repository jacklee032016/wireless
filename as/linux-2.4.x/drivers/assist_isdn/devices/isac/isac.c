/*
 * $Id: isac.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
...
MODULE_LICENSE("GPL");
#endif
EXPORT_SYMBOL(AS_ISDN_isac_init);
EXPORT_SYMBOL(AS_ISDN_isac_free);
EXPORT_SYMBOL(AS_ISDN_isac_interrupt);
EXPORT_SYMBOL(AS_ISDN_clear_isac);
EXPORT_SYMBOL(AS_ISDN_ISAC_l1hw);
#endif
...
ph_command(dchannel_t *dch, unsigned int command)
{
	if (dch->debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&dch->inst, "ph_command %x", command);
...
isac_empty_fifo(dchannel_t *dch, int count)
{
	...
	if ((dch->debug & L1_DEB_ISAC) && !(dch->debug & L1_DEB_ISAC_FIFO))
		AS_ISDN_debugprint(&dch->inst, "isac_empty_fifo");
	...
	if ((dch->rx_skb->len + count) >= MAX_DFRAME_LEN_L1) {
		if (dch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&dch->inst, "isac_empty_fifo overrun %d",
...
isac_fill_fifo(dchannel_t *dch)
{
	...
	if ((dch->debug & L1_DEB_ISAC) && !(dch->debug & L1_DEB_ISAC_FIFO))
		AS_ISDN_debugprint(&dch->inst, "isac_fill_fifo");
	...
	if (test_and_set_bit(FLG_DBUSY_TIMER, &dch->DFlags)) {
		AS_ISDN_debugprint(&dch->inst, "isac_fill_fifo dbusytimer running");
...
isac_rme_irq(dchannel_t *dch)
{
	...
	if ((val & 0x70) != 0x20) {
		if (val & 0x40) {
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "ISAC RDO");
		...
		if (!(val & 0x20)) {
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "ISAC CRC error");
#ifdef ERROR_STATISTIC
			dch->err_crc++;
#endif
		}
...
isac_retransmit(dchannel_t *dch)
{
	...
	if (test_bit(FLG_TX_BUSY, &dch->DFlags)) {
		...
		printk(KERN_WARNING "AS_ISDN: ISAC XDU no TX_BUSY\n");
		AS_ISDN_debugprint(&dch->inst, "ISAC XDU no TX_BUSY");
		if (test_and_clear_bit(FLG_TX_NEXT, &dch->DFlags)) {
			if (dch->next_skb) {
				dch->tx_len = dch->next_skb->len;
				memcpy(dch->tx_buf,
...
isac_mos_irq(dchannel_t *dch)
{
	...
	if (dch->debug & L1_DEB_MONITOR)
		AS_ISDN_debugprint(&dch->inst, "ISAC MOSR %02x", val);
#if ARCOFI_USE
	if (val & 0x08) {
		if (!isac->mon_rx) {
			if (!(isac->mon_rx = kmalloc(MAX_MON_FRAME, GFP_ATOMIC))) {
				if (dch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&dch->inst, "ISAC MON RX out of memory!");
				isac->mocr &= 0xf0;
				isac->mocr |= 0x0a;
		...
		if (isac->mon_rxp >= MAX_MON_FRAME) {
			...
			isac->mon_rxp = 0;
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "ISAC MON RX overflow!");
			goto afterMONR0;
		}
		isac->mon_rx[isac->mon_rxp++] = dch->read_reg(dch->inst.data, ISAC_MOR0);
		if (dch->debug & L1_DEB_MONITOR)
			AS_ISDN_debugprint(&dch->inst, "ISAC MOR0 %02x", isac->mon_rx[isac->mon_rxp -1]);
		if (isac->mon_rxp == 1) {
			isac->mocr |= 0x04;
			dch->write_reg(dch->inst.data, ISAC_MOCR, isac->mocr);
		}
...
afterMONR0:
	if (val & 0x80) {
		...
		if (dch->debug & L1_DEB_MONITOR)
			AS_ISDN_debugprint(&dch->inst, "ISAC MOR1 %02x", isac->mon_rx[isac->mon_rxp -1]);
		isac->mocr |= 0x40;
		dch->write_reg(dch->inst.data, ISAC_MOCR, isac->mocr);
	}
afterMONR1:
	...
	if (val & 0x02) {
		...
		if (dch->debug & L1_DEB_MONITOR)
			AS_ISDN_debugprint(&dch->inst, "ISAC %02x -> MOX0", isac->mon_tx[isac->mon_txp -1]);
	}
AfterMOX0:
	if (val & 0x20) {
		if ((!isac->mon_tx) || (isac->mon_txc && 
		...
		isac->mon_tx[isac->mon_txp++]);
		if (dch->debug & L1_DEB_MONITOR)
			AS_ISDN_debugprint(&dch->inst, "ISAC %02x -> MOX1", isac->mon_tx[isac->mon_txp -1]);
	}
AfterMOX1:
...
isac_cisq_irq(dchannel_t *dch) {
	...
	if (dch->debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&dch->inst, "ISAC CIR0 %02X", val);
	if (val & 2) {
		if (dch->debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&dch->inst, "ph_state change %x->%x",
	...
	if (val & 1) {
		...
		if (dch->debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&dch->inst, "ISAC CIR1 %02X", val );
	}
}
...
isacsx_cic_irq(dchannel_t *dch)
{
	...
	if (dch->debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&dch->inst, "ISACSX CIR0 %02X", val);
...
isacsx_rme_irq(dchannel_t *dch)
{
	...
	if ((val & (ISACSX_RSTAD_VFR | 
	    ...
	    != (ISACSX_RSTAD_VFR | ISACSX_RSTAD_CRC)) {
	    	if (dch->debug & L1_DEB_WARN)
	    		AS_ISDN_debugprint(&dch->inst, "RSTAD %#x, dropped", val);
#ifdef ERROR_STATISTIC
...
void
AS_ISDN_isac_interrupt(dchannel_t *dch, u_char val)
{
	if (dch->debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&dch->inst, "ISAC interrupt %02x", val);
	if (dch->type & ISAC_TYPE_ISACSX) {
		...
		if (val & ISACSX_ISTA_ICD) {
			...
			if (dch->debug & L1_DEB_ISAC)
				AS_ISDN_debugprint(&dch->inst, "ISTAD %02x", val);
			if (val & ISACSX_ISTAD_XDU) {
				if (dch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&dch->inst, "ISAC XDU");
#ifdef ERROR_STATISTIC
				dch->err_tx++;
#endif
				isac_retransmit(dch);
			...
			if (val & ISACSX_ISTAD_XMR) {
				if (dch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&dch->inst, "ISAC XMR");
#ifdef ERROR_STATISTIC
				dch->err_tx++;
#endif
				isac_retransmit(dch);
			...
			if (val & ISACSX_ISTAD_RFO) {
				if (dch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&dch->inst, "ISAC RFO");
				dch->write_reg(dch->inst.data, ISACSX_CMDRD, ISACSX_CMDRD_RMC);
			}
		...
		if (val & 0x20)	/* RSC - never */
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "ISAC RSC interrupt");
		if (val & 0x02)	/* SIN - never */
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "ISAC SIN interrupt");
		if (val & 0x01) {	/* EXI */
			val = dch->read_reg(dch->inst.data, ISAC_EXIR);
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "ISAC EXIR %02x", val);
			if (val & 0x80)	/* XMR */
				AS_ISDN_debugprint(&dch->inst, "ISAC XMR");
			if (val & 0x40) { /* XDU */
				if (dch->debug & L1_DEB_WARN)
...
int
AS_ISDN_ISAC_l1hw(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	dchannel_t	*dch;
	int		ret = -EINVAL;
	AS_ISDN_head_t	*hh;
	...
	if (hh->prim == PH_DATA_REQ) {
		if (dch->next_skb) {
			AS_ISDN_debugprint(&dch->inst, " l2l1 next_skb exist this shouldn't happen");
			return(-EBUSY);
		}
		dch->inst.lock(dch->inst.data,0);
		...
		if (hh->dinfo == HW_RESET) {
			...
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "isac_l1hw unknown ctrl %x",
					hh->dinfo);
			ret = -EINVAL;
		}
		dch->inst.unlock(dch->inst.data);
		...
		if (dch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&dch->inst, "isac_l1hw unknown prim %x",
				hh->prim);
		ret = -EINVAL;
	}
	if (!ret)
...
void 
AS_ISDN_isac_free(dchannel_t *dch) {
	isac_chip_t     *isac = dch->hw;
	...
	if (test_bit(FLG_DBUSY_TIMER, &dch->DFlags)) {
		...
		if (rbch & ISAC_RBCH_XAC) { /* D-Channel Busy */
			...
			if (dch->tx_idx) {
				...
				printk(KERN_WARNING "AS_ISDN: ISAC D-Channel Busy no tx_idx\n");
...
int
AS_ISDN_isac_init(dchannel_t *dch)
{
	isac_chip_t	*isac = dch->hw;
	u_char		val;
	...
	if (dch->type & ISAC_TYPE_ISACSX) {
		...
		printk(KERN_INFO "AS_ISDN_isac_init: ISACSX\n");
		dchannel_sched_event(dch, D_L1STATECHANGE);
		ph_command(dch, ISAC_CMD_RS);
	} else { /* old isac */
	  	dch->write_reg(dch->inst.data, ISAC_MASK, 0xff);
		val = dch->read_reg(dch->inst.data, ISAC_RBCH);
		printk(KERN_INFO "AS_ISDN_isac_init: ISAC version (%x): %s\n", val, ISACVer[(val >> 5) & 3]);
		dch->type |= ((val >> 5) & 3);
		if (ISAC_TYPE_IOM1 & dch->type) {
			/* IOM 1 Mode */
...
void
AS_ISDN_clear_isac(dchannel_t *dch)
{
	isac_chip_t	*isac = dch->hw;
	u_int		val, eval;
	...
	AS_ISDN_debugprint(&dch->inst, "ISAC STAR %x", val);
	val = dch->read_reg(dch->inst.data, ISAC_MODE);
	AS_ISDN_debugprint(&dch->inst, "ISAC MODE %x", val);
	val = dch->read_reg(dch->inst.data, ISAC_ADF2);
	AS_ISDN_debugprint(&dch->inst, "ISAC ADF2 %x", val);
	val = dch->read_reg(dch->inst.data, ISAC_ISTA);
	AS_ISDN_debugprint(&dch->inst, "ISAC ISTA %x", val);
	if (val & 0x01) {
		eval = dch->read_reg(dch->inst.data, ISAC_EXIR);
		AS_ISDN_debugprint(&dch->inst, "ISAC EXIR %x", eval);
	}
	val = dch->read_reg(dch->inst.data, ISAC_CIR0);
	AS_ISDN_debugprint(&dch->inst, "ISAC CIR0 %x", val);
	dch->ph_state = (val >> 2) & 0xf;
}
