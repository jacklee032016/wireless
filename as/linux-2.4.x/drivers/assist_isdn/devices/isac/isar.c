/*
 * $Id: isar.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
...
waitforHIA(bchannel_t *bch, int timeout)
{
	...
	if (!timeout)
		printk(KERN_WARNING "AS_ISDN: ISAR waitforHIA timeout\n");
	return(timeout);
}
	...
	u_char *msg)
{
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "sendmsg(%02x,%02x,%d)", his, creg, len);
#endif
	...
	if (msg && len) {
		...
		if (bch->debug & L1_DEB_HSCX_FIFO) {
			...
			while (i>0) {
				...
				AS_ISDN_QuickHex(t, &msg[len-i], (i>64) ? 64:i);
				AS_ISDN_debugprint(&bch->inst, bch->blog);
				i -= 64;
			}
		}
#endif
	...
	if (msg && ireg->clsb) {
		...
		if (bch->debug & L1_DEB_HSCX_FIFO) {
			...
			while (i>0) {
				...
				AS_ISDN_QuickHex(t, &msg[ireg->clsb-i], (i>64) ? 64:i);
				AS_ISDN_debugprint(&bch->inst, bch->blog);
				i -= 64;
			}
		}
...
#if DUMP_MBOXFRAME
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "rcv_mbox(%02x,%02x,%d)", ireg->iis, ireg->cmsb,
			ireg->clsb);
#endif
}
...
#define	BLK_HEAD_SIZE 6
	...
	bch->debug &= ~(L1_DEB_HSCX | L1_DEB_HSCX_FIFO);
#endif
	printk(KERN_ERR "isar_load_firmware buf %#lx\n", (u_long)buf);
	printk(KERN_ERR "isar_load_firmware size: %d\n", size);
	cnt = 0;
	/* disable ISAR IRQ */
	bch->Write_Reg(bch->inst.data, 0, ISAR_IRQBIT, 0);
	...
	while (cnt < size) {
		...
		cnt += BLK_HEAD_SIZE;
		p += BLK_HEAD_SIZE;
		printk(KERN_ERR "isar firmware block (%#x,%5d,%#x)\n",
		...
		while (left>0) {
			...
			*mp++ = noc;
			...
			sp = (u_short *)tmpmsg;
#if DBG_LOADFIRM
			printk(KERN_ERR "isar: load %3d words at %04x\n",
				 noc, sadr);
#endif
			sadr += noc;
			while(noc) {
		...
		printk(KERN_ERR "isar firmware block %5d words loaded\n",
			blk_head->len);
	}
	/* 10ms delay */
	cnt = 10;
	...
	if ((ih->reg->iis != ISAR_IIS_STDSP) || ih->reg->cmsb || len) {
		...
		printk(KERN_ERR "isar start dsp success\n");
	/* NORMAL mode entered */
	/* Enable IRQs of ISAR */
	...
	while ((!ih->reg->bstat) && cnt) {
 	...
 	if (!cnt) {
		...
		printk(KERN_ERR "isar general status event %x\n",
			ih->reg->bstat);
	}
	/* 10ms delay */
	cnt = 10;
	...
	if ((ih->reg->cmsb == ISAR_CTRL_STST) && (ih->reg->clsb == 1)
		&& (ih->reg->par[0] == 0)) {
		printk(KERN_ERR "isar selftest OK\n");
	} else {
		printk(KERN_ERR "isar selftest not OK %x/%x/%x\n",
	...
	if (!cnt) {
		...
		if ((ih->reg->cmsb == ISAR_CTRL_SWVER) && (ih->reg->clsb == 1))
			printk(KERN_ERR "isar software version %#x\n",
				ih->reg->par[0]);
		else {
...
{
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "HL->LL FAXIND %x", status);
...
isar_rcv_frame(bchannel_t *bch)
{
	...
	if (!ih->reg->clsb) {
		AS_ISDN_debugprint(&bch->inst, "isar zero len frame");
		bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
		return;
	}
	switch (bch->protocol) {
	    case ISDN_PID_NONE:
		AS_ISDN_debugprint(&bch->inst, "isar protocol 0 spurious IIS_RDATA %x/%x/%x",
		...
		if ((skb = alloc_stack_skb(ih->reg->clsb, bch->up_headerlen))) {
			...
			printk(KERN_WARNING "AS_ISDN: skb out of memory\n");
			bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
		}
		break;
		...
		if ((bch->rx_idx + ih->reg->clsb) > MAX_DATA_MEM) {
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "isar_rcv_frame: incoming packet too large");
			bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
			bch->rx_idx = 0;
		} else if (ih->reg->cmsb & HDLC_ERROR) {
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "isar frame error %x len %d",
			...
			if (ih->reg->cmsb & HDLC_FED) {
				if (bch->rx_idx < 3) { /* last 2 bytes are the FCS */
					if (bch->debug & L1_DEB_WARN)
						AS_ISDN_debugprint(&bch->inst, "isar frame to short %d",
							bch->rx_idx);
	...
	case ISDN_PID_L1_B_T30FAX:
		if (ih->state != STFAX_ACTIV) {
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "isar_rcv_frame: not ACTIV");
			bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
			bch->rx_idx = 0;
			break;
		}
		if (ih->cmd == PCTRL_CMD_FRM) {
			...
			bch->rx_idx = ih->reg->clsb;
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "isar_rcv_frame: %d", bch->rx_idx);
			if ((skb = alloc_stack_skb(bch->rx_idx, bch->up_headerlen))) {
				...
				if (ih->reg->cmsb & SART_NMD) { /* ABORT */
					if (bch->debug & L1_DEB_WARN)
						AS_ISDN_debugprint(&bch->inst, "isar_rcv_frame: no more data");
					bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
					bch->rx_idx = 0;
...
//					set_skb_flag(skb, DF_NOMOREDATA);
		...
		if (ih->cmd != PCTRL_CMD_FRH) {
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "isar_rcv_frame: unknown fax mode %x",
					ih->cmd);
			bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
			bch->rx_idx = 0;
			break;
...
isar_fill_fifo(bchannel_t *bch)
{
	...
	if ((bch->debug & L1_DEB_HSCX) && !(bch->debug & L1_DEB_HSCX_FIFO))
		AS_ISDN_debugprint(&bch->inst, "%s", __FUNCTION__);
	...
	if (!bch->tx_idx) {
		if (bch->debug & L1_DEB_HSCX)
			AS_ISDN_debugprint(&bch->inst, "frame start");
		if ((bch->protocol == ISDN_PID_L1_B_T30FAX) &&
			...
			if (count > 1) {
				if ((ptr[0]== 0xff) && (ptr[1] == 0x13)) {
					...
					if (bch->debug & L1_DEB_HSCX)
						AS_ISDN_debugprint(&bch->inst, "set LASTDATA");
					if (msb == HDLC_FED)
	...
	switch (bch->protocol) {
		...
		case ISDN_PID_L1_B_T30FAX:
			if (ih->state != STFAX_ACTIV) {
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "%s: not ACTIV",
						__FUNCTION__);
				...
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "%s: not FTH/FTM",
						__FUNCTION__);
			}
			break;
		default:
			if (bch->debug)
				AS_ISDN_debugprint(&bch->inst, "%s: protocol(%x) error",
...
isar_pump_status_rsp(bchannel_t *bch, isar_reg_t *ireg) {
	...
	if (ril > 14) {
		if (bch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&bch->inst, "wrong pstrsp ril=%d",ril);
		ril = 15;
	}
	switch(ireg->par[1]) {
		case 0:
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "pump strsp %s", bch->conmsg);
}
...
isar_pump_statev_modem(bchannel_t *bch, u_char devt) {
	...
	switch(devt) {
		case PSEV_10MS_TIMER:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev TIMER");
			break;
		case PSEV_CON_ON:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev CONNECT");
			bch_sched_event(bch, B_LL_CONNECT);
			break;
		case PSEV_CON_OFF:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev NO CONNECT");
			sendmsg(bch, dps | ISAR_HIS_PSTREQ, 0, 0, NULL);
			bch_sched_event(bch, B_LL_NOCARRIER);
			break;
		case PSEV_V24_OFF:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev V24 OFF");
			break;
		case PSEV_CTS_ON:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev CTS ON");
			break;
		case PSEV_CTS_OFF:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev CTS OFF");
			break;
		case PSEV_DCD_ON:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev CARRIER ON");
			test_and_set_bit(ISAR_RATE_REQ, &ih->reg->Flags);
			sendmsg(bch, dps | ISAR_HIS_PSTREQ, 0, 0, NULL);
			break;
		case PSEV_DCD_OFF:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev CARRIER OFF");
			break;
		case PSEV_DSR_ON:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev DSR ON");
			break;
		case PSEV_DSR_OFF:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev DSR_OFF");
			break;
		case PSEV_REM_RET:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev REMOTE RETRAIN");
			break;
		case PSEV_REM_REN:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev REMOTE RENEGOTIATE");
			break;
		case PSEV_GSTN_CLR:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev GSTN CLEAR", devt);
			break;
		default:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "unknown pump stev %x", devt);
			break;
	}
}
...
isar_pump_statev_fax(bchannel_t *bch, u_char devt) {
	...
	switch(devt) {
		...
		case PSEV_RSP_READY:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev RSP_READY");
...
//			}
		...
		case PSEV_LINE_TX_H:
			if (ih->state == STFAX_LINE) {
				if (bch->debug & L1_DEB_HSCX)
					AS_ISDN_debugprint(&bch->inst, "pump stev LINE_TX_H");
				ih->state = STFAX_CONT;
				sendmsg(bch, dps | ISAR_HIS_PUMPCTRL, PCTRL_CMD_CONT, 0, NULL);
			} else {
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "pump stev LINE_TX_H wrong st %x",
						ih->state);
			}
			break;
		case PSEV_LINE_RX_H:
			if (ih->state == STFAX_LINE) {
				if (bch->debug & L1_DEB_HSCX)
					AS_ISDN_debugprint(&bch->inst, "pump stev LINE_RX_H");
				ih->state = STFAX_CONT;
				sendmsg(bch, dps | ISAR_HIS_PUMPCTRL, PCTRL_CMD_CONT, 0, NULL);
			} else {
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "pump stev LINE_RX_H wrong st %x",
						ih->state);
			}
			break;
		case PSEV_LINE_TX_B:
			if (ih->state == STFAX_LINE) {
				if (bch->debug & L1_DEB_HSCX)
					AS_ISDN_debugprint(&bch->inst, "pump stev LINE_TX_B");
				ih->state = STFAX_CONT;
				sendmsg(bch, dps | ISAR_HIS_PUMPCTRL, PCTRL_CMD_CONT, 0, NULL);
			} else {
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "pump stev LINE_TX_B wrong st %x",
						ih->state);
			}
			break;
		case PSEV_LINE_RX_B:
			if (ih->state == STFAX_LINE) {
				if (bch->debug & L1_DEB_HSCX)
					AS_ISDN_debugprint(&bch->inst, "pump stev LINE_RX_B");
				ih->state = STFAX_CONT;
				sendmsg(bch, dps | ISAR_HIS_PUMPCTRL, PCTRL_CMD_CONT, 0, NULL);
			} else {
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "pump stev LINE_RX_B wrong st %x",
						ih->state);
			}
			break;
		case PSEV_RSP_CONN:
			if (ih->state == STFAX_CONT) {
				if (bch->debug & L1_DEB_HSCX)
					AS_ISDN_debugprint(&bch->inst, "pump stev RSP_CONN");
				...
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "pump stev RSP_CONN wrong st %x",
						ih->state);
			}
			break;
		case PSEV_FLAGS_DET:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev FLAGS_DET");
			break;
		case PSEV_RSP_DISC:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev RSP_DISC state(%d)", ih->state);
			if (ih->state == STFAX_ESCAPE) {
				...
				switch(ih->newcmd) {
					...
					default:
						if (bch->debug & L1_DEB_HSCX)
							AS_ISDN_debugprint(&bch->inst, "RSP_DISC unknown newcmd %x", ih->newcmd);
						break;
				}
		...
		case PSEV_RSP_SILDET:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev RSP_SILDET");
		...
		case PSEV_RSP_SILOFF:
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev RSP_SILOFF");
			break;
		case PSEV_RSP_FCERR:
			if (ih->state == STFAX_LINE) {
				if (bch->debug & L1_DEB_HSCX)
					AS_ISDN_debugprint(&bch->inst, "pump stev RSP_FCERR try %d",
						ih->try_mod);
				if (ih->try_mod--) {
					sendmsg(bch, dps | ISAR_HIS_PUMPCTRL,
						ih->cmd, 1,
			...
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "pump stev RSP_FCERR");
			ih->state = STFAX_ESCAPE;
	...
	switch (ih->reg->iis & ISAR_IIS_MSCMSD) {
		case ISAR_IIS_RDATA:
			if ((bc = sel_bch_isar(bch, ih->reg->iis >> 6))) {
				...
				AS_ISDN_debugprint(&bch->inst, "isar spurious IIS_RDATA %x/%x/%x",
					ih->reg->iis, ih->reg->cmsb, ih->reg->clsb);
				bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
			}
			break;
...
#endif
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "Buffer STEV dpath%d msb(%x)",
		...
		case ISAR_IIS_PSTEV:
			if ((bc = sel_bch_isar(bch, ih->reg->iis >> 6))) {
				...
				if (bc->protocol == ISDN_PID_L1_B_MODEM_ASYNC) {
					...
					if (bch->debug & L1_DEB_WARN)
						AS_ISDN_debugprint(&bch->inst, "isar IIS_PSTEV pmode %d stat %x",
							bc->protocol, ih->reg->cmsb);
				}
			} else {
				AS_ISDN_debugprint(&bch->inst, "isar spurious IIS_PSTEV %x/%x/%x",
					ih->reg->iis, ih->reg->cmsb, ih->reg->clsb);
				bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
			}
			break;
		case ISAR_IIS_PSTRSP:
			if ((bc = sel_bch_isar(bch, ih->reg->iis >> 6))) {
				...
				AS_ISDN_debugprint(&bch->inst, "isar spurious IIS_PSTRSP %x/%x/%x",
					ih->reg->iis, ih->reg->cmsb, ih->reg->clsb);
				bch->Write_Reg(bch->inst.data, 1, ISAR_IIA, 0);
			}
			break;
		...
		case ISAR_IIS_IOM2RSP:
			...
			if ((bch->debug & (L1_DEB_HSCX | L1_DEB_HSCX_FIFO))
				...
				AS_ISDN_QuickHex(tp, (u_char *)ih->reg->par, ih->reg->clsb);
				AS_ISDN_debugprint(&bch->inst, debbuf);
			}
			break;
		case ISAR_IIS_INVMSG:
			rcv_mbox(bch, ih->reg, debbuf);
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "invalid msg his:%x",
					ih->reg->cmsb);
			break;
		default:
			rcv_mbox(bch, ih->reg, debbuf);
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "unhandled msg iis(%x) ctrl(%x/%x)",
					ih->reg->iis, ih->reg->cmsb, ih->reg->clsb);
			break;
	}
}
...
ftimer_handler(bchannel_t *bch) {
	if (bch->debug)
		AS_ISDN_debugprint(&bch->inst, "ftimer flags %04x",
			bch->Flag);
...
modeisar(bchannel_t *bch, int channel, u_int bprotocol, u_char *param)
{
		...
		switch (bprotocol) {
			...
			case ISDN_PID_L2_B_TRANSDTMF:
				...
				if (!test_and_set_bit(ISAR_DP1_USE,
					...
					AS_ISDN_debugprint(&bch->inst, "isar modeisar analog funktions only with DP1");
					return(-EBUSY);
				}
				break;
		}
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "isar dp%d protocol %x->%x ichan %d",
...
isar_pump_cmd(bchannel_t *bch, int cmd, u_char para)
{
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "isar_pump_cmd %x/%x state(%x)",
					cmd, para, ih->state);
	switch(cmd) {
		case HW_MOD_FTM:
...
int
isar_down(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	bchannel_t	*bch;
	int		ret = -EINVAL;
	AS_ISDN_head_t	*hh;
	...
	if ((hh->prim == PH_DATA_REQ) ||
		...
		if (bch->next_skb) {
			AS_ISDN_debugprint(&bch->inst, " l2l1 next_skb exist this shouldn't happen");
			return(-EBUSY);
		}
		bch->inst.lock(bch->inst.data, 0);
		...
		val = (int *)skb->data;
		if (bch->debug & L1_DEB_HSCX)
			AS_ISDN_debugprint(&bch->inst, "PH_CONTROL | REQUEST %x/%x",
					hh->dinfo, *val);
...
int init_isar(bchannel_t *bch)
{
	...
	printk(KERN_INFO "AS_ISDN: ISAR driver Rev. %s\n", AS_ISDN_getrev(ISAR_revision));
	bch->hw_bh = isar_bh;
