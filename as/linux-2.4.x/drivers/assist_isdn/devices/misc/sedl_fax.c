/*
 * $Id: sedl_fax.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
...
{
	register AS_ISDN_HWlock_t	*lock = &((sedl_fax *)data)->lock;
...
do_sedl_interrupt(sedl_fax *sf)
{
	...
	val = readreg(sf->addr, sf->isac, ISAC_ISTA);
      Start_ISAC:
	if (val)
		AS_ISDN_isac_interrupt(&sf->dch, val);
	val = readreg(sf->addr, sf->isar, ISAR_IRQBIT);
	if ((val & ISAR_IRQSTA) && cnt) {
		cnt--;
		if (sf->dch.debug & L1_DEB_HSCX)
			printk(KERN_ERR  "ISAR IntStat after IntRoutine cpu%d\n",
				smp_processor_id());
		goto Start_ISAR;
	}
	val = readreg(sf->addr, sf->isac, ISAC_ISTA);
	if (val && cnt) {
		...
		if (sf->dch.debug & L1_DEB_ISAC)
			printk(KERN_ERR  "ISAC IntStat after IntRoutine cpu%d\n",
				smp_processor_id());
		goto Start_ISAC;
	}
	if (!cnt)
		if (sf->dch.debug & L1_DEB_ISAC)
			printk(KERN_ERR  "Sedlbauer IRQ LOOP\n");
...
static int init_card(sedl_fax *sf)
{
	...
	while (cnt) {
		...
		AS_ISDN_clear_isac(&sf->dch);
		if ((ret=AS_ISDN_isac_init(&sf->dch))) {
...
static int sedl_cnt;
static AS_ISDNobject_t	speedfax;
static int debug;
...
speedfax_manager(void *data, u_int prim, void *arg) {
	...
	switch(prim) {
		...
		if (channel==2)
			return(AS_ISDN_SetIF(inst, arg, prim, AS_ISDN_ISAC_l1hw, NULL, &card->dch));
		else
			return(AS_ISDN_SetIF(inst, arg, prim, isar_down, NULL, &card->bch[channel]));
...
static int __devinit setup_instance(sedl_fax *card)
{
	...
	AS_ISDN_init_instance(&card->dch.inst, &speedfax, card);
	sprintf(card->dch.inst.name, "SpeedFax%d", sedl_cnt+1);
	AS_ISDN_set_dchannel_pid(&pid, protocol[sedl_cnt], layermask[sedl_cnt]);
	AS_ISDN_init_dch(&card->dch);
	for (i=0; i<2; i++) {
		card->bch[i].channel = i;
		AS_ISDN_init_instance(&card->bch[i].inst, &speedfax, card);
	...
	printk(KERN_ERR  "sfax card %p dch %p bch1 %p bch2 %p\n",
...
static int __devinit sedlpci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	printk(KERN_INFO "AS_ISDN: sedlpci found adapter %s at %s\n",
...
#endif
{
	...
	printk(KERN_INFO "AS_ISDN: sedlpnp_probe found adapter %s at IO %#x irq %d\n",
...
static int __init Speedfax_init(void)
{
	...
	if ((err = as_isdn_register(&speedfax))) {
...
static void __exit Speedfax_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&speedfax))) {
