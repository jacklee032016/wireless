/*
 * $Id: w6692.c,v 1.1.1.1 2006/11/29 08:55:13 lizhijie Exp $
...
{
	register AS_ISDN_HWlock_t	*lock = &((w6692pci *)data)->lock;
...
ph_command(w6692pci *card, u_char command)
{
	if (card->dch.debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&card->dch.inst, "ph_command %x", command);
...
W6692_empty_Dfifo(w6692pci *card, int count)
{
	...
	if ((dch->debug & L1_DEB_ISAC) && !(dch->debug & L1_DEB_ISAC_FIFO))
		AS_ISDN_debugprint(&dch->inst, "empty_Dfifo");

	if (!dch->rx_skb) {
		if (!(dch->rx_skb = alloc_stack_skb(MAX_DFRAME_LEN_L1, dch->up_headerlen))) {
			printk(KERN_WARNING "AS_ISDN: D receive out of memory\n");
	...
	if ((dch->rx_skb->len + count) >= MAX_DFRAME_LEN_L1) {
		if (dch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&dch->inst, "empty_Dfifo overrun %d",
...
W6692_fill_Dfifo(w6692pci *card)
{
	...
	if ((dch->debug & L1_DEB_ISAC) && !(dch->debug & L1_DEB_ISAC_FIFO))
		AS_ISDN_debugprint(&dch->inst, "fill_Dfifo");
	...
	if (test_and_set_bit(FLG_DBUSY_TIMER, &dch->DFlags)) {
		AS_ISDN_debugprint(&dch->inst, "fill_Dfifo dbusytimer running");
...
d_retransmit(w6692pci *card)
{
	...
	if (test_bit(FLG_TX_BUSY, &dch->DFlags)) {
		...
		printk(KERN_WARNING "AS_ISDN: w6692 XDU no TX_BUSY\n");
		AS_ISDN_debugprint(&dch->inst, "XDU no TX_BUSY");
	...
	if (stat & (W_D_RSTA_RDOV | W_D_RSTA_CRCE | W_D_RSTA_RMB)) {
		if (stat & W_D_RSTA_RDOV) {
			if (card->dch.debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&card->dch.inst, "D-channel RDOV");
		...
		if (stat & W_D_RSTA_CRCE) {
			if (card->dch.debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&card->dch.inst, "D-channel CRC error");
#ifdef ERROR_STATISTIC
			card->dch.err_crc++;
#endif
		}
		if (stat & W_D_RSTA_RMB) {
			if (card->dch.debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&card->dch.inst, "D-channel ABORT");
#ifdef ERROR_STATISTIC
			card->dch.err_rx++;
#endif
		}
...
handle_statusD(w6692pci *card) {
	...
	if (card->dch.debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&card->dch.inst, "D_EXIR %02x", exval);
	if (exval & (W_D_EXI_XDUN | W_D_EXI_XCOL)) {	/* Transmit underrun/collision */
		if (card->dch.debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&card->dch.inst, "D-channel underrun/collision");
#ifdef ERROR_STATISTIC
		dch->err_tx++;
#endif
		d_retransmit(card);
	...
	if (exval & W_D_EXI_TIN2)	/* TIN2 - never */
		if (card->dch.debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&card->dch.inst, "spurious TIN2 interrupt");
	if (exval & W_D_EXI_MOC) {	/* MOC - not supported */
		v1 = ReadW6692(card, W_MOSR);
		if (card->dch.debug & L1_DEB_ISAC) {
			AS_ISDN_debugprint(&card->dch.inst, "spurious MOC interrupt");
			AS_ISDN_debugprint(&card->dch.inst, "MOSR %02x", v1);
		}
	}
	if (exval & W_D_EXI_ISC) {	/* ISC - Level1 change */
		cir = ReadW6692(card, W_CIR);
		if (card->dch.debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&card->dch.inst, "ISC CIR=0x%02X", cir);
		if (cir & W_CIR_ICC) {
			v1 = cir & W_CIR_COD_MASK;
			if (card->dch.debug & L1_DEB_ISAC)
				AS_ISDN_debugprint(&card->dch.inst, "ph_state_change %x -> %x",
					dch->ph_state, v1);
			dch->ph_state = v1;
		...
		if (cir & W_CIR_SCC) {
			...
			if (card->dch.debug & L1_DEB_ISAC)
				AS_ISDN_debugprint(&card->dch.inst, "SCC SQR=0x%02X", v1);
		}
	}
	if (exval & W_D_EXI_WEXP) {
		if (card->dch.debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&card->dch.inst, "spurious WEXP interrupt!");
	}
	if (exval & W_D_EXI_TEXP) {
		if (card->dch.debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&card->dch.inst, "spurious TEXP interrupt!");
	}
}
...
W6692_empty_Bfifo(bchannel_t *bch, int count)
{
	...
	if ((bch->debug & L1_DEB_HSCX) && !(bch->debug & L1_DEB_HSCX_FIFO))
		AS_ISDN_debugprint(&bch->inst, "empty_Bfifo %d", count);
	if (bch->protocol == ISDN_PID_NONE) {
		if (bch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&bch->inst, "empty_Bfifo ISDN_PID_NONE");
	...
	if (bch->rx_idx + count > MAX_DATA_MEM) {
		if (bch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&bch->inst, "empty_Bfifo incoming packet too large");
		WriteW6692B(card, bch->channel, W_B_CMDR, W_B_CMDR_RACK | W_B_CMDR_RACT);
		bch->rx_idx = 0;
		return;
	}
...
W6692_fill_Bfifo(bchannel_t *bch)
{
	...
	if ((bch->debug & L1_DEB_HSCX) && !(bch->debug & L1_DEB_HSCX_FIFO))
		AS_ISDN_debugprint(&bch->inst, "%s: %d/%d", __FUNCTION__,
			count, bch->tx_idx);
	bch->tx_idx += count;
...
mode_w6692(bchannel_t *bch, int bc, int protocol)
{
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "B%d protocol %x-->%x ch %d-->%d",
...
W6692B_interrupt(w6692pci *card, int ch)
{
	...
	if (bch->debug & L1_DEB_HSCX)
		AS_ISDN_debugprint(&bch->inst, "ch%d stat %#x", bch->channel, stat);
	...
	if (stat & W_B_EXI_RME) {
		...
		if (star & (W_B_STAR_RDOV | W_B_STAR_CRCE | W_B_STAR_RMB)) {
			if ((star & W_B_STAR_RDOV) && (bch->protocol != ISDN_PID_NONE)) {
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "B%d RDOV protocol=%x",
						ch +1, bch->protocol);
#ifdef ERROR_STATISTIC
				bch->err_rdo++;
#endif
			...
			if ((star & W_B_STAR_CRCE) && (bch->protocol == ISDN_PID_L1_B_64HDLC)) {
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "B%d CRC error", ch +1);
#ifdef ERROR_STATISTIC
				bch->err_crc++;
#endif
			}
			if ((star & W_B_STAR_RMB) && (bch->protocol == ISDN_PID_L1_B_64HDLC)) {
				if (bch->debug & L1_DEB_WARN)
					AS_ISDN_debugprint(&bch->inst, "B%d message abort", ch +1);
#ifdef ERROR_STATISTIC
				bch->err_inv++;
#endif
			}
			...
			if (bch->rx_idx > 0) {
				if (bch->debug & L1_DEB_HSCX)
					AS_ISDN_debugprint(&bch->inst, "Bchan Frame %d", bch->rx_idx);
	...
	if (stat & W_B_EXI_RDOV) {
		if (!(star & W_B_STAR_RDOV)) { /* only if it is not handled yet */
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "B%d RDOV IRQ protocol=%x",
					ch +1, bch->protocol);
#ifdef ERROR_STATISTIC
			bch->err_rdo++;
#endif
	...
	if (stat & W_B_EXI_XFR) {
		if (!(stat & (W_B_EXI_RME | W_B_EXI_RMR))) {
			star = ReadW6692B(card, ch, W_B_STAR);
			if (bch->debug & L1_DEB_HSCX)
				AS_ISDN_debugprint(&bch->inst, "B%d star %02x", ch +1, star);
		}
		if (star & W_B_STAR_XDOW) {
			if (bch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&bch->inst, "B%d XDOW protocol=%x",
					ch +1, bch->protocol);
#ifdef ERROR_STATISTIC
			bch->err_xdu++;
#endif
	...
	if (stat & W_B_EXI_XDUN) {
		if (bch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&bch->inst, "B%d XDUN protocol=%x",
				ch +1, bch->protocol);
#ifdef ERROR_STATISTIC
		bch->err_xdu++;
#endif
...
/* Begin IRQ handler */
	if (card->dch.debug & L1_DEB_ISAC)
		AS_ISDN_debugprint(&card->dch.inst, "ista %02x", ista);
	ista &= ~card->imask;
	...
	if (ista & (W_INT_XINT0 | W_INT_XINT1)) /* XINT0/1 - never */
		AS_ISDN_debugprint(&card->dch.inst, "W6692 spurious XINT!");
/* End IRQ Handler */
	spin_lock_irqsave(&card->lock.lock, flags);
#ifdef SPIN_DEBUG
...
dbusy_timer_handler(dchannel_t *dch)
{
	...
	if (test_bit(FLG_DBUSY_TIMER, &dch->DFlags)) {
		...
		if (dch->debug) 
			AS_ISDN_debugprint(&dch->inst, "D-Channel Busy RBCH %02x STAR %02x",
				rbch, star);
		if (star & W_D_STAR_XBZ) {	/* D-Channel Busy */
			...
			if (dch->tx_idx) {
				...
				printk(KERN_WARNING "AS_ISDN: W6692 D-Channel Busy no tx_idx\n");
				AS_ISDN_debugprint(&dch->inst, "D-Channel Busy no tx_idx");
			}
			/* Transmitter reset */
...
void initW6692(w6692pci *card)
{
	...
	if (card->pots) {
		...
		if (card->dch.debug & L1_DEB_ISAC)
			AS_ISDN_debugprint(&card->dch.inst, "W_XADDR: %02x", val);
...
/******************************/
static int
w6692_l2l1B(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	bchannel_t	*bch;
	int		ret = -EINVAL;
	AS_ISDN_head_t	*hh;
...
static int
w6692_l1hwD(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	dchannel_t	*dch;
	int		ret = -EINVAL;
	AS_ISDN_head_t	*hh;
	...
	if (hh->prim == PH_DATA_REQ) {
		if (dch->next_skb) {
			AS_ISDN_debugprint(&dch->inst, "w6692 l2l1 next_skb exist this shouldn't happen");
			return(-EBUSY);
		}
		dch->inst.lock(dch->inst.data,0);
		...
		if (hh->dinfo == HW_RESET) {
			...
			if (dch->debug & L1_DEB_WARN)
				AS_ISDN_debugprint(&dch->inst, "w6692_l1hw unknown ctrl %x",
					hh->dinfo);
			ret = -EINVAL;
		}
		dch->inst.unlock(dch->inst.data);
		...
		if (dch->debug & L1_DEB_WARN)
			AS_ISDN_debugprint(&dch->inst, "w6692_l1hw unknown prim %x",
				hh->prim);
		ret = -EINVAL;
	}
	if (!ret)
...
setup_w6692(w6692pci *card)
{
	...
	if (!request_region(card->addr, 256, "w6692")) {
		printk(KERN_WARNING
		       "AS_ISDN: %s config port %x-%x already in use\n",
		       "w6692",
		       card->addr,
		       card->addr + 255);
		return(-EIO);
	...
	if (debug)
		printk(KERN_ERR  "W6692 ISTA=0x%X\n", val);
	val = ReadW6692(card, W_IMASK);
	if (debug)
		printk(KERN_ERR  "W6692 IMASK=0x%X\n", val);
	val = ReadW6692(card, W_D_EXIR);
	if (debug)
		printk(KERN_ERR  "W6692 D_EXIR=0x%X\n", val);
	val = ReadW6692(card, W_D_EXIM);
	if (debug)
		printk(KERN_ERR  "W6692 D_EXIM=0x%X\n", val);
	val = ReadW6692(card, W_D_RSTA);
	if (debug)
		printk(KERN_ERR  "W6692 D_RSTA=0x%X\n", val);
	return (0);
}
...
w6692_manager(void *data, u_int prim, void *arg) {
	...
	switch(prim) {
		...
		if (channel==2)
			return(AS_ISDN_SetIF(inst, arg, prim, w6692_l1hwD, NULL,
				&card->dch));
		else
			return(AS_ISDN_SetIF(inst, arg, prim, w6692_l2l1B, NULL,
				&card->bch[channel]));
		break;
...
static int __devinit setup_instance(w6692pci *card)
{
	...
	AS_ISDN_init_instance(&card->dch.inst, &w6692, card);
	sprintf(card->dch.inst.name, "W6692_%d", w6692_cnt+1);
	AS_ISDN_set_dchannel_pid(&pid, protocol[w6692_cnt], layermask[w6692_cnt]);
	AS_ISDN_init_dch(&card->dch);
	card->dch.hw = card;
	for (i=0; i<2; i++) {
		card->bch[i].channel = i;
		AS_ISDN_init_instance(&card->bch[i].inst, &w6692, card);
	...
	if (debug)
		printk(KERN_ERR  "w6692 card %p dch %p bch1 %p bch2 %p\n",
...
static int __devinit w6692_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	printk(KERN_INFO "AS_ISDN_w6692: found adapter %s at %s\n",
...
static void __exit w6692_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&w6692))) {
