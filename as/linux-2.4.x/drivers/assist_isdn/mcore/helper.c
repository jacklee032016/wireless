...
/* whether this AS_ISDNobject with support this protocol : layer, B/D etc
 return 0 : found 
*/
int AS_ISDN_HasProtocol(AS_ISDNobject_t *obj, u_int protocol)
{
	int layer;
	u_int pmask;
...
*/
int AS_ISDN_SetHandledPID(AS_ISDNobject_t *obj, AS_ISDN_pid_t *pid)
{
	int layer;
	int ret = 0;
	AS_ISDN_pid_t sav;
...
int AS_ISDN_get_protocol(AS_ISDNstack_t *st, int layer)
{
	if (!st)
	{
		int_error();
...
int AS_ISDN_SetIF(AS_ISDNinstance_t *owner, AS_ISDNif_t *hif, u_int prim, void *upfunc,
...
*/
int AS_ISDN_ConnectIF(AS_ISDNinstance_t *owner, AS_ISDNinstance_t *peer)
{
	AS_ISDNif_t *hif;
...
int AS_ISDN_DisConnectIF(AS_ISDNinstance_t *inst, AS_ISDNif_t *hif)
{
	if (hif)
	{
		if (hif->clone)
...
void AS_ISDN_init_instance(AS_ISDNinstance_t *inst, AS_ISDNobject_t *obj, void *data)
{
	if (!obj)
	{
		int_error();
