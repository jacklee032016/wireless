...
extern int	AS_ISDN_get_protocol(AS_ISDNstack_t *, int);
extern int	AS_ISDN_HasProtocol(AS_ISDNobject_t *, u_int);
extern int	AS_ISDN_SetHandledPID(AS_ISDNobject_t *, AS_ISDN_pid_t *);
extern void	AS_ISDN_RemoveUsedPID(AS_ISDN_pid_t *, AS_ISDN_pid_t *);
extern void	AS_ISDN_init_instance(AS_ISDNinstance_t *, AS_ISDNobject_t *, void *);
...
static inline int
AS_ISDN_HasProtocolP(AS_ISDNobject_t *obj, int *PP)
{
	if (!PP) {
		int_error();
		return(-EINVAL);
...
static inline int if_newhead(AS_ISDNif_t *i, u_int prim, int dinfo, struct sk_buff *skb)
{
	if (!i->func || !skb)
		return(-ENXIO);
...
static inline int
__mid_if_link(AS_ISDNif_t *i, u_int prim, int dinfo, int len, void *arg, int reserve, char *fn, int line)
{
	struct sk_buff	*skb;
	int		err;
...
#else
static inline int
if_link(AS_ISDNif_t *i, u_int prim, int dinfo, int len, void *arg, int reserve)
{
	struct sk_buff	*skb;
	int		err;
