...
static int  get_free_stackid(AS_ISDNstack_t *mst, int flag)
{
	u_int		id=1, found;
	AS_ISDNstack_t	*st;
...
AS_ISDNstack_t *get_stack4id(u_int id)
{
	AS_ISDNstack_t *cst, *st;
...
AS_ISDNlayer_t * getlayer4lay(AS_ISDNstack_t *st, int layermask)
{
...
*/
AS_ISDNinstance_t *get_instance(AS_ISDNstack_t *st, int layer_nr, int protocol)
{
	AS_ISDNlayer_t	*layer;
...
AS_ISDNinstance_t *get_instance4id(u_int id)
{
	AS_ISDNinstance_t *inst;
...
int get_layermask(AS_ISDNlayer_t *layer)
{
	int mask = 0;
...
int insertlayer(AS_ISDNstack_t *st, AS_ISDNlayer_t *layer, int layermask)
{
	AS_ISDNlayer_t *item;
...
AS_ISDNstack_t *new_stack(AS_ISDNstack_t *master, AS_ISDNinstance_t *inst)
{
	AS_ISDNstack_t *newst;
	...
	if (!(newst = kmalloc(sizeof(AS_ISDNstack_t), GFP_ATOMIC)))
	{
		printk(KERN_ERR "kmalloc asISDN_stack failed\n");
		return(NULL);
	}
	memset(newst, 0, sizeof(AS_ISDNstack_t));
...
*/
static int release_layers(AS_ISDNstack_t *st, u_int prim)
{
	AS_ISDNinstance_t *inst;
...
int do_for_all_layers(AS_ISDNstack_t *st, u_int prim, void *arg)
{
	AS_ISDNinstance_t *inst;
	AS_ISDNlayer_t    *layer, *nl;
	int		cnt = 0;
...
int change_stack_para(AS_ISDNstack_t *st, u_int prim, AS_ISDN_stPara_t *stpara)
{
	int	changed = 0;
...
int release_stack(AS_ISDNstack_t *st)
{
	int err;
	AS_ISDNstack_t *cst, *nst;
...
*/
void release_stacks(AS_ISDNobject_t *obj)
{
	AS_ISDNstack_t *st, *tmp;
	AS_ISDNlayer_t *layer, *ltmp;
	int rel;
...
static void get_free_instid(AS_ISDNstack_t *st, AS_ISDNinstance_t *inst)
{
	AS_ISDNinstance_t *il;
	...
	else
	{
		list_for_each_entry(il, &AS_ISDN_instlist, list)
		{
			if (il->id == inst->id)
			{
				...
				inst->id += INST_ID_INC;
				il = list_entry(AS_ISDN_instlist.next, AS_ISDNinstance_t, list);
			}
		}
	}
}
...
*/
int register_layer(AS_ISDNstack_t *st, AS_ISDNinstance_t *inst)
{
	AS_ISDNlayer_t	*layer = NULL;
	int		refinc = 0;
	...
	if (st)
	{
		...
		else if (!(layer = kmalloc(sizeof(AS_ISDNlayer_t), GFP_ATOMIC)))
		...
		memset(layer, 0, sizeof(AS_ISDNlayer_t));
...
*/
int unregister_instance(AS_ISDNinstance_t *inst)
{
	AS_ISDNlayer_t *layer;
	int err = 0;
...
int set_stack(AS_ISDNstack_t *st, AS_ISDN_pid_t *pid)
{
	int 		err;
	u_char		*pbuf = NULL;
	AS_ISDNinstance_t	*inst;
...
*/
int clear_stack(AS_ISDNstack_t *st)
{
...
static int test_stack_protocol(AS_ISDNstack_t *st, u_int l1prot, u_int l2prot, u_int l3prot)
{
...
int evaluate_stack_pids(AS_ISDNstack_t *st, AS_ISDN_pid_t *pid)
{
	int 		err;
