/*
 * $Id: layer3.c,v 1.1.1.1 2006/11/29 08:55:14 lizhijie Exp $
...
void newl3state(l3_process_t *pc, int state)
{
	if (pc->l3->debug & L3_DEB_STATE)
		l3m_debug(&pc->l3->l3m, "newstate cr %d %d --> %d", pc->callref & 0x7F, pc->state, state);
	pc->state = state;
}
...
l3_process_t *new_l3_process(layer3_t *l3, int cr/*callref*/, int n303, u_int id)
{
	l3_process_t	*p = NULL;
...
static void l3ml3p(layer3_t *l3, int pr)
{
	...
	list_for_each_entry_safe(p, np, &l3->plist, list) 
		l3->p_mgr(p, pr, NULL);	/* point to dss1man */
