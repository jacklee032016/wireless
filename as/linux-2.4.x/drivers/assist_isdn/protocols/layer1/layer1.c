/*
 * $Id: layer1.c,v 1.1.1.1 2006/11/29 08:55:14 lizhijie Exp $
...
static int l1from_up(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	layer1_t	*l1;
	AS_ISDN_head_t	*hh;
	int		err = 0;
...
static int l1from_down(AS_ISDNif_t *hif,  struct sk_buff *skb)
{
	layer1_t	*l1;
	AS_ISDN_head_t	*hh;
	int		err = 0;
...
{
	AS_ISDNinstance_t	*inst = &l1->inst;
...
static int new_l1(AS_ISDNstack_t *st, AS_ISDN_pid_t *pid)
{
	layer1_t *nl1;
	int err;
...
int Isdnl1Init(void)
{
	...
	printk(KERN_INFO AS_VERSION_INFO("L1 Driver") );
#ifdef MODULE
