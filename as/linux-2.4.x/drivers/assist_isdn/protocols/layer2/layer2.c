/*
 * $Id: layer2.c,v 1.1.1.1 2006/11/29 08:55:14 lizhijie Exp $
 */
...
static int l2_chain_down(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	if (!hif || !hif->fdata)
		return(-EINVAL);
...
static int ph_data_confirm(AS_ISDNif_t *up, AS_ISDN_head_t *hh, struct sk_buff *skb)
{
	layer2_t *l2 = up->fdata;
	struct sk_buff *nskb = skb; 
	AS_ISDNif_t *next = up->clone;
	int ret = -EAGAIN;
...
static int l2from_down(AS_ISDNif_t *hif, struct sk_buff *askb)
{
	layer2_t	*l2;
	int 		ret = -EINVAL;
...
static int l2from_up(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	layer2_t	*l2;
	AS_ISDN_head_t	*hh;
	int		ret = -EINVAL;
...
{
	AS_ISDNinstance_t  *inst = &l2->inst;
...
static int new_l2(AS_ISDNstack_t *st, AS_ISDN_pid_t *pid, layer2_t **newl2)
{
	layer2_t *nl2;
	int err;
	u_char *p;
...
static int clone_l2(layer2_t *l2, AS_ISDNinstance_t **new_ip)
{
	int err;
	layer2_t	*nl2;
	AS_ISDNif_t	*nif;
	...
	if (!new_ip)
		return(-EINVAL);
	st = (AS_ISDNstack_t *)*new_ip;
	if (!st)
		return(-EINVAL);
	if (!l2->inst.down.peer)
		return(-EINVAL);

	if (!(nif = kmalloc(sizeof(AS_ISDNif_t), GFP_ATOMIC)))
	{
	...
	memset(nif, 0, sizeof(AS_ISDNif_t));
	nl2->cloneif = nif;
...
int Isdnl2_Init(void)
{
	...
	printk(KERN_INFO AS_VERSION_INFO( "L2 Driver") );
#ifdef MODULE
