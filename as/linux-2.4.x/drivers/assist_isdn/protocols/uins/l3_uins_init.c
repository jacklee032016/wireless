/*
 * $Id: l3_uins_init.c,v 1.1.1.1 2006/11/29 08:55:14 lizhijie Exp $
...
static AS_ISDNobject_t uins_obj;

extern int ins_fromup(AS_ISDNif_t *hif, struct sk_buff *skb);
extern int ins_fromdown(AS_ISDNif_t *hif, struct sk_buff *skb);
...
static int new_uins_layer3(AS_ISDNstack_t *st, AS_ISDN_pid_t *pid)
{
	layer3_t	*nl3;
	int		err;
...
int UINS_Init(void)
{
	...
	printk(KERN_INFO AS_VERSION_INFO("UserINS Rev") );
#ifdef MODULE
