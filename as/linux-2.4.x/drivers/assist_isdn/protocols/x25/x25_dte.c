/*
 * $Id: x25_dte.c,v 1.1.1.1 2006/11/29 08:55:14 lizhijie Exp $
 * Linux modular ISDN subsystem, AS_ISDN
...
static AS_ISDNobject_t x25dte_obj;
...
r_llready(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_R1);
	if (test_and_clear_bit(X25_STATE_ESTABLISH, &l3->state))
	{
		as_isdn_fsm_event(&l3->x25r, EV_L3_RESTART_REQ, NULL);
	}
}
...
r_r0_restart_l3(struct FsmInst *fi, int event, void *arg)
{
	...
	as_isdn_fsm_event(&l3->l2l3m, EV_L3_ESTABLISH_REQ, NULL);
}
...
r_restart_l3(struct FsmInst *fi, int event, void *arg)
{
	...
	if (arg)
		memcpy(l3->cause, arg, 2);
	fsm_change_state(fi, ST_R2);
	l3->TRval = T20_VALUE;
	l3->TRrep = R20_VALUE;
	X25sendL3frame(NULL, l3, X25_PTYPE_RESTART, 2, l3->cause);
	fsm_add_timer(&l3->TR, l3->TRval, EV_L3_RESTART_TOUT, NULL, 0);
}
...
r_restart_cnf_err(struct FsmInst *fi, int event, void *arg)
{
	...
	if (fi->state == ST_R3)
		cause[1] = 19;
	as_isdn_fsm_event(&l3->x25r, EV_L3_RESTART_REQ, cause);
}
...
r_timeout(struct FsmInst *fi, int event, void *arg)
{
	...
	if (l3->TRrep) {
		X25sendL3frame(NULL, l3, X25_PTYPE_RESTART, 2, l3->cause);
		fsm_restart_timer(&l3->TR, l3->TRval, EV_L3_RESTART_TOUT, NULL, 0);
		l3->TRrep--;
	} else {
		fsm_del_timer(&l3->TR, 0);
		fsm_change_state(fi, ST_R1);
...
p_p0_ready(struct FsmInst *fi, int event, void *arg)
{
	...
	if (test_bit(X25_STATE_PERMANENT, &l3c->state)) {
		fsm_change_state(fi, ST_P4);
		/* connect */
		as_isdn_fsm_event(&l3c->x25d, EV_L3_CONNECT, NULL);
	} else {
		fsm_change_state(fi, ST_P1);
		if (test_bit(X25_STATE_ORGINATE, &l3c->state))
			as_isdn_fsm_event(fi, EV_L3_OUTGOING_CALL, NULL);
	}
}
...
p_outgoing(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_add_timer(&l3c->TP, T21_VALUE, EV_L3_CALL_TOUT, NULL, 0);
}
...
p_call_timeout(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_P6);
	l3c->cause[0] = 0;
	l3c->cause[1] = 49;
	l3c->TPval = T23_VALUE;
	l3c->TPrep = R23_VALUE;
	fsm_add_timer(&l3c->TP, l3c->TPval, EV_L3_CLEAR_TOUT, NULL, 0);
...
d_reset_req(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_D2);
	l3c->TDval = T22_VALUE;
	l3c->TDrep = R22_VALUE;
	fsm_add_timer(&l3c->TD, l3c->TDval, EV_L3_RESET_TOUT, NULL, 0);
	if (skb) {
		if (skb->len >= 2) {
			memcpy(l3c->cause, skb->data, 2);
...
d_reset_timeout(struct FsmInst *fi, int event, void *arg)
{
	...
	if (l3c->TDrep) {
		...
		l3c->cause[0] = 0;
		l3c->cause[1] = 51;
		fsm_change_state(fi, ST_D0);
		if (test_bit(X25_STATE_PERMANENT, &l3c->state))
			X25_clear_connection(l3c, NULL, 0x3303); 
		else
			as_isdn_fsm_event(&l3c->x25p, EV_L3_CLEARING, NULL);
	}
}
...
dte_data_ind_d(x25_channel_t *chan, struct sk_buff *skb, u_char gfi, u_char ptype)
{
	...
	if (event != -1) {
		if (event == EV_L3_RESETING)
			as_isdn_fsm_event(&chan->x25d, event, NULL);
		else
			as_isdn_fsm_event(&chan->x25d, event, skb);
		return(X25_ERRCODE_DISCARD);
	}
	switch (ptype) {
		case X25_PTYPE_INTERRUPT:
			if (test_and_set_bit(X25_STATE_DXE_INTSENT, &chan->state)) {
				chan->cause[0] = 0;
				chan->cause[1] = 44;
				as_isdn_fsm_event(&chan->x25d, EV_L3_RESETING, NULL);
			} else {
...
dte_data_ind_p(x25_l3_t *l3, struct sk_buff *skb, u_char gfi, __u16 channel, u_char ptype)
{
	...
	if (ptype == X25_PTYPE_CALL) {
		...
		if (chan) {
			as_isdn_fsm_event(&chan->x25p, EV_L2_INCOMING_CALL, skb);
		} else {
	...
	if (event == EV_L3_CLEARING)
		as_isdn_fsm_event(&chan->x25p, event, NULL);
	else
		as_isdn_fsm_event(&chan->x25p, event, skb);
	return(ret);
}
...
dte_data_ind_r(x25_l3_t *l3, struct sk_buff *skb, u_char gfi, __u16 channel, u_char ptype)
{
	...
	if ((ptype == X25_PTYPE_RESTART) || (ptype == X25_PTYPE_RESTART_CNF)) {
		...
		if (ptype == X25_PTYPE_RESTART)
			as_isdn_fsm_event(&l3->x25r, EV_L2_RESTART, skb);
		else
			as_isdn_fsm_event(&l3->x25r, EV_L2_RESTART_CNF, skb);
	} else {
		if (l3->x25r.state == ST_R1)
			return(dte_data_ind_p(l3, skb, gfi, channel, ptype));
...
static int
dte_from_down(AS_ISDNif_t *hif,  struct sk_buff *skb)
{
	x25_l3_t	*l3;
	AS_ISDN_head_t	*hh;
	int		ret = -EINVAL;
	...
	if (l3->debug)
		printk(KERN_ERR  "%s: prim(%x) dinfo(%x)\n", __FUNCTION__, hh->prim, hh->dinfo);
	switch(hh->prim) {
		case DL_DATA_IND:
		...
		case DL_ESTABLISH_CNF:
			ret = as_isdn_fsm_event(&l3->l2l3m, EV_LL_ESTABLISH_CNF, NULL);
			if (ret) {
				int_error();
			}
			ret = 0;
		...
		case DL_ESTABLISH_IND:
			ret = as_isdn_fsm_event(&l3->l2l3m, EV_LL_ESTABLISH_IND, NULL);
			if (ret) {
				int_error();
			}
			ret = 0;
			...
			break;
		case DL_RELEASE_CNF:
			ret = as_isdn_fsm_event(&l3->l2l3m, EV_LL_RELEASE_CNF, NULL);
			if (ret) {
				int_error();
			}
			ret = 0;
			...
			break;
		case DL_RELEASE_IND:
			ret = as_isdn_fsm_event(&l3->l2l3m, EV_LL_RELEASE_IND, NULL);
			if (ret) {
				int_error();
			}
			ret = 0;
...
static int
new_l3(AS_ISDNstack_t *st, AS_ISDN_pid_t *pid) {
	x25_l3_t	*n_l3;
	int		err;
...
static int
dte_from_up(AS_ISDNif_t *hif, struct sk_buff *skb)
{
	x25_l3_t	*l3;
	x25_channel_t   *l3c;
	AS_ISDN_head_t	*hh;
		...
		case CAPI_CONNECT_B3_REQ:
			...
			if (l3c) {
				...
				if (l3->x25r.state == ST_R0)
					as_isdn_fsm_event(&l3->x25r, EV_L3_RESTART_REQ, "\000\000");
				else
					err = as_isdn_fsm_event(&l3c->x25p, EV_L3_OUTGOING_CALL, NULL);
				if (err)
		...
		case CAPI_RESET_B3_REQ:
			if (l3c) {
				...
				if (skb->len <= 4) { // default NCPI
					l3c->cause[0] = 0;
					l3c->cause[1] = 0;
					err = as_isdn_fsm_event(&l3c->x25d, EV_L3_RESETING, NULL);
				} else {
					skb_pull(skb, 4);
					err = as_isdn_fsm_event(&l3c->x25d, EV_L3_RESETING, skb);
					skb_push(skb, 4);
				}
				if (err)
					info = 0x2001;
		...
		case CAPI_DISCONNECT_B3_REQ:
			if (l3c) {
				...
				if (skb->len <= 4) { // default NCPI
					l3c->cause[0] = 0;
					l3c->cause[1] = 0;
					err = as_isdn_fsm_event(&l3c->x25p, EV_L3_CLEARING, NULL);
				} else {
					skb_pull(skb, 4);
					err = as_isdn_fsm_event(&l3c->x25p, EV_L3_CLEARING, skb);
					skb_push(skb, 4);
				}
				if (err)
					info = 0x2001;
		...
		case CAPI_CONNECT_B3_RESP:
			if (l3c) {
				...
				if (skb->len <= 4) { // default NCPI
					l3c->cause[0] = 0;
					l3c->cause[1] = 0;
					err = as_isdn_fsm_event(&l3c->x25p, event, NULL);
				} else {
					skb_pull(skb, 4);
					err = as_isdn_fsm_event(&l3c->x25p, event, skb);
				}
			} else {
				skb_push(skb, 4);
...
dte_manager(void *data, u_int prim, void *arg) {
	...
	if (debug & DEBUG_L3X25_MGR)
		printk(KERN_ERR  "l3x25_manager data:%p prim:%x arg:%p\n", data, prim, arg);
	if (!data)
		return(err);
	...
	switch(prim) {
		...
		return(AS_ISDN_SetIF(inst, arg, prim, dte_from_up, dte_from_down, l3_l));
		...
		if (debug & DEBUG_L3X25_MGR)
			printk(KERN_ERR  "X25_release_l3 id %x\n", l3_l->inst.st->id);
		X25_release_l3(l3_l);
		break;
...
x25_dte_init(void)
{
	...
	printk(KERN_INFO AS_VERSION_INFO("X25 DTE module") );
#ifdef MODULE
	...
	if ((err = as_isdn_register(&x25dte_obj)))
	{
	...
	else
	{
		...
		as_isdn_fsm_new(&dte_rfsm, RFnList, R_FN_COUNT);
		...
		as_isdn_fsm_new(&dte_pfsm, PFnList, P_FN_COUNT);
		...
		as_isdn_fsm_new(&dte_dfsm, DFnList, D_FN_COUNT);
	}
	return(err);
}

static void x25_dte_cleanup(void)
{
	x25_l3_t	*l3, *nl3;
	int		err;

	if ((err = as_isdn_unregister(&x25dte_obj))) 
	{
	...
	X25_l3_cleanup();
	as_isdn_fsm_free(&dte_rfsm);
	as_isdn_fsm_free(&dte_pfsm);
	as_isdn_fsm_free(&dte_dfsm);
}
