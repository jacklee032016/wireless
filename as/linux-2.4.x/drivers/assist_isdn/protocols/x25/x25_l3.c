/*
 * $Id: x25_l3.c,v 1.1.1.1 2006/11/29 08:55:14 lizhijie Exp $
...
ll_activate(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_LL_ESTAB_WAIT);
...
ll_connect(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_LL_ESTAB);
	as_isdn_fsm_event(&l3->x25r, EV_LL_READY, NULL);
...
ll_release_req(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_LL_REL_WAIT);
...
X25_reset_channel(x25_channel_t *l3c, struct sk_buff *skb)
{
	...
	if (skb) {
		if (skb->len == 2)
			...
			printk(KERN_ERR  "%s: skb len not 2 (%d)\n", __FUNCTION__, skb->len);
	}
...
X25_restart(x25_l3_t *l3)
{
	...
	list_for_each_entry(l3c, &l3->channellist, list) {
		...
		as_isdn_fsm_event(&l3c->x25p, EV_L3_READY, NULL);
	}
	return(0);
}
...
X25_receive_data(x25_channel_t *chan, int ps, int flag, struct sk_buff *skb)
{
	...
	if (skb_headroom(skb) < CAPI_B3_DATA_IND_HEADER_SIZE) {
		printk(KERN_ERR  "%s: only %d bytes headroom, need %d",
...
X25_get_and_test_pr(x25_channel_t *chan, u_char ptype, struct sk_buff *skb)
{
	...
	pr = pr_m >> 1;
	if (chan->debug)
		printk(KERN_ERR  "%s: pr(%d) chan: pr(%d) ps(%d)\n",
...
X25_get_and_test_ps(x25_channel_t *chan, u_char ptype, struct sk_buff *skb)
{
	...
	if (chan->debug)
		printk(KERN_ERR  "%s: ps(%d) chan: rps(%d)\n",
...
X25_release_channel(x25_channel_t *l3c)
{
	...
	fsm_del_timer(&l3c->TP, 1);
	fsm_del_timer(&l3c->TD, 2);
	discard_queue(&l3c->dataq);
	discard_confq(l3c);
	if (l3c->confq) {
		kfree(l3c->confq);
...
int
new_x25_l3(x25_l3_t **l3_p, AS_ISDNstack_t *st, AS_ISDN_pid_t *pid, AS_ISDNobject_t *obj, int debug) {
	x25_l3_t	*n_l3;
	int		err;
	...
	n_l3->entity = AS_ISDN_ENTITY_NONE;
	n_l3->next_id = 1;
	spin_lock_init(&n_l3->lock);
	memcpy(&n_l3->inst.pid, pid, sizeof(AS_ISDN_pid_t));
	AS_ISDN_init_instance(&n_l3->inst, obj, n_l3);
	if (!AS_ISDN_SetHandledPID(obj, &n_l3->inst.pid)) {
		int_error();
		kfree(n_l3);
		return(-ENOPROTOOPT);
	}
	...
	fsm_init_timer(&n_l3->x25r, &n_l3->TR);
...
X25sendL3frame(x25_channel_t *l3c, x25_l3_t *l3, u_char pt, int len, void *arg)
{
	...
	AS_ISDN_sethead(DL_DATA_REQ, X25_next_id(l3), skb);

	if (l3->l2l3m.state == ST_LL_ESTAB) {
		AS_ISDNif_t	*down = &l3->inst.down;
...
X25_invoke_sending(x25_channel_t *l3c)
{
	...
	while(cq && skb) {
		...
		if (l3c->l3->l2l3m.state == ST_LL_ESTAB)
			...
			AS_ISDN_sethead(DL_DATA_REQ, X25_next_id(l3c->l3), nskb);	
...
X25_l3_init(void)
{
	...
	as_isdn_fsm_new(&llfsm, LLFnList, LL_FN_COUNT);
	return(0);
}
