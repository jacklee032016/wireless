/*
* $Id: ieee80211_crypto_tkip.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 */
...
tkip_encap(struct ieee80211_key *k, struct sk_buff *skb, u_int8_t keyid)
{
	...
	if (ic->ic_flags & IEEE80211_F_COUNTERM) {
		...
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO,
			...
			swjtu_mesh_mac_sprintf(wh->i_addr2), __func__);
...
tkip_enmic(struct ieee80211_key *k, struct sk_buff *skb)
{
	...
	if (k->wk_flags & IEEE80211_KEY_SWMIC) {
		...
		IEEE80211_DPRINTF(ctx->tc_ic, IEEE80211_MSG_CRYPTO,
			"%s: [%s] doing SWMIC\n",
			__func__, swjtu_mesh_mac_sprintf(wh->i_addr1));
		struct ieee80211com *ic = ctx->tc_ic;
		int hdrlen;
		u_int8_t *mic;
		...
		if (skb_tailroom(skb) < tkip.ic_miclen) {
			...
			IEEE80211_DPRINTF(ctx->tc_ic, IEEE80211_MSG_CRYPTO,
				...
				swjtu_mesh_mac_sprintf(wh->i_addr1), skb_tailroom(skb));
			/* XXX statistic */
			return 0;
		}
	...
	struct sk_buff *skb, int hdrlen)
{
	...
	if (skb_tailroom(skb) < tkip.ic_trailer) {
		...
		IEEE80211_DPRINTF(ctx->tc_ic, IEEE80211_MSG_CRYPTO,
			...
			swjtu_mesh_mac_sprintf(wh->i_addr1), tkip.ic_name,
			skb_tailroom(skb));
		/* XXX statistic */
		return 0;
	}
