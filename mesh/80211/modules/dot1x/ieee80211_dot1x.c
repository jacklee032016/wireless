/*
* $Id: ieee80211_dot1x.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 */
	...
	struct sk_buff *skb)
{
	...
	if (eap->eap_id != ean->ean_currentId) {
		IEEE80211_DPRINTF(ean->ean_ic, IEEE80211_MSG_DOT1X,
			...
			swjtu_mesh_mac_sprintf(ean->ean_node->ni_macaddr),
...
eapol_input(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt)
{
	...
	if (ni == NULL) {
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_DOT1X,
		    ...
		    swjtu_mesh_mac_sprintf(skb->mac.ethernet->h_source),
		    eapol->eapol_type));
	...
	if (ean == NULL && eapol->eapol_type != EAPOL_TYPE_START) {
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_DOT1X,
		    ...
		    swjtu_mesh_mac_sprintf(ni->ni_macaddr), eapol->eapol_type));
...
txReqId(struct eapol_auth_node *ean)
{
	...
	IEEE80211_DPRINTF(ean->ean_ic, IEEE80211_MSG_DOT1X,
		("[%s] %s currentId %u\n",
		swjtu_mesh_mac_sprintf(ean->ean_node->ni_macaddr), __func__,
		ean->ean_currentId));
...
eapol_send_simple(struct eapol_node *en, u_int8_t type)
{
	...
	if (skb == NULL) {
		IEEE80211_DPRINTF(en->en_ic, IEEE80211_MSG_ANY,
			("[%s] could not allocate sk_buff (%s) \n",
			swjtu_mesh_mac_sprintf(en->en_node->ni_macaddr), __func__));
		return;
	}
