 ...
 *
 *	$Id: br_stp.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 *
...
void br_transmit_config(struct net_bridge_port *p)
{
	...
	if (fwd_timer_is_running(&p->hold_timer)) {
		p->config_pending = 1;
		return;
	}
	...
	if (!br_is_root_bridge(br)) {
		...
		age = fwd_timer_get_residue(&root->message_age_timer) + 1;
	...
	p->config_pending = 0;
	fwd_timer_set(&p->hold_timer, jiffies);
}
...
static void br_record_config_information(struct net_bridge_port *p, struct br_config_bpdu *bpdu)
{
	...
	fwd_timer_set(&p->message_age_timer, jiffies - bpdu->message_age);
}
...
void br_topology_change_detection(struct net_bridge *br)
{
	...
	if (br_is_root_bridge(br)) {
		...
		fwd_timer_set(&br->topology_change_timer, jiffies);
