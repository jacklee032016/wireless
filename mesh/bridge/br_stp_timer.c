 ...
 *
 *	$Id: br_stp_timer.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 *
...
static void br_hello_timer_expired(struct net_bridge *br)
{
	...
	fwd_timer_set(&br->hello_timer, jiffies);
}
...
static void br_forward_delay_timer_expired(struct net_bridge_port *p)
{
	if (p->state == BR_STATE_LISTENING) {
		...
		fwd_timer_set(&p->forward_delay_timer, jiffies);
...
static void br_tcn_timer_expired(struct net_bridge *br)
{
	...
	br_transmit_tcn(br);
	fwd_timer_set(&br->tcn_timer, jiffies);
}
...
{
	if (fwd_timer_has_expired(&p->message_age_timer, p->br->max_age)) {
		fwd_timer_clear(&p->message_age_timer);
		br_message_age_timer_expired(p);
	}

	if (fwd_timer_has_expired(&p->forward_delay_timer, p->br->forward_delay)) {
		fwd_timer_clear(&p->forward_delay_timer);
		br_forward_delay_timer_expired(p);
	}

	if (fwd_timer_has_expired(&p->hold_timer, MESH_FWD_HOLD_TIME)) {
		fwd_timer_clear(&p->hold_timer);
...
static void br_check_timers(struct net_bridge *br)
{
	...
	if (fwd_timer_has_expired(&br->gc_timer, br->gc_interval)) {
		fwd_timer_set(&br->gc_timer, jiffies);
		mesh_fwd_db_cleanup(br);
	}

	if (fwd_timer_has_expired(&br->hello_timer, br->hello_time)) {
		fwd_timer_clear(&br->hello_timer);
		br_hello_timer_expired(br);
	}

	if (fwd_timer_has_expired(&br->tcn_timer, br->bridge_hello_time)) {
		fwd_timer_clear(&br->tcn_timer);
		br_tcn_timer_expired(br);
	}

	if (fwd_timer_has_expired(&br->topology_change_timer, br->bridge_forward_delay + br->bridge_max_age)) {
		fwd_timer_clear(&br->topology_change_timer);
