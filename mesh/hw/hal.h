/*
* $Id: hal.h,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 */
...
/* Transmit queue flags that control various operational parameters. */
typedef enum
{
	...
	TXQ_FLAG_RDYTIME_EXP_POLICY_ENABLE 		= 0x0040, 	/* enable ready time expiry policy */
	TXQ_FLAG_FRAG_BURST_BACKOFF_ENABLE 	= 0x0080		/* enable backoff while	sending fragment burst*/
} HAL_TX_QUEUE_FLAGS;
...
typedef struct
{
	...
	struct
	{
		...
		u_int8_t		shortPreamble;	/* mask for enabling short preamble in CCK rate code */
		u_int8_t		dot11Rate;	/* value for supported rates info element of MLME */
		u_int8_t		controlRate;	/* index of next lower basic rate; used for dur. calcs */
...
#define	HAL_ABI_VERSION	0x04112900	/* YYMMDDnn */
	...
	/* Reset functions */
	HAL_BOOL  __ahdecl(*ah_reset)(struct ath_hal *, HAL_OPMODE,	HAL_CHANNEL *, HAL_BOOL bChannelChange, HAL_STATUS *status);
	...
	HAL_BOOL  __ahdecl(*ah_updateCTSForBursting)(struct ath_hal *,struct ath_desc *, struct ath_desc *,	struct ath_desc *, struct ath_desc *,	u_int32_t, u_int32_t);
	...
	/* Misc Functions */
	HAL_STATUS __ahdecl(*ah_getCapability)(struct ath_hal *,	HAL_CAPABILITY_TYPE, u_int32_t capability,u_int32_t *result);
	HAL_BOOL   __ahdecl(*ah_setCapability)(struct ath_hal *, HAL_CAPABILITY_TYPE, u_int32_t capability,u_int32_t setting, HAL_STATUS *);
	...
	void	  __ahdecl(*ah_beaconInit)(struct ath_hal *, u_int32_t nexttbtt, u_int32_t intval);
