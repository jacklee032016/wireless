/*
* $Id: phy_isr.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 */
...
{
	MESH_DEVICE *mdev = (MESH_DEVICE *)dev_id;
	...
	if ((mdev->flags & (MESHF_RUNNING|MESHF_UP)) != (MESHF_RUNNING|MESHF_UP))
	{
	...
	else
	{
...
#ifdef HAS_VMAC
		{
			...
			{
				if (ath_cu_softmac_handle_rx(mdev,1))
				{
...
#ifdef HAS_VMAC
		{
			...
			{
				if (ath_cu_softmac_handle_txdone(mdev,1))
				{
					ATH_SCHEDULE_TQUEUE(&sc->sc_vmac_worktq, &needmark);
				}
			}
