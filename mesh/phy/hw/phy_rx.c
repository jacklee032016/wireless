/*
* $Id: phy_rx.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 */
...
static void ath_rx_capture(MESH_DEVICE *dev, struct ath_desc *ds, struct sk_buff *skb)
{
...
#define	PA2DESC(_sc, _pa) \
	...
	do
	{
		...
		if (bf == NULL)
		{		/* XXX ??? can this happen */
			mdev_printf(dev, "%s: no buffer!\n", __func__);
			break;
		}
		ds = bf->bf_desc;
		if (ds->ds_link == bf->bf_daddr)
		{/* ds_link is phy address of next desc; bf_daddr is phy address of this desc */
		...
		/* after check, remove this ath_buff from sc_rxbuf queue */
		STAILQ_REMOVE_HEAD(&sc->sc_rxbuf, bf_list);
		if (ds->ds_rxstat.rs_more)
		{/* frame spans multiple descriptors are not support */
			/*
...
#if 0					
					ieee80211_notify_michael_failure(ic, (struct ieee80211_frame *) skb->data,
					    sc->sc_splitmic ?ds->ds_rxstat.rs_keyix-32 :ds->ds_rxstat.rs_keyix);
#endif
				}
			}
...
rx_accept:
		...
		KASSERT(len <= skb_tailroom(skb), 	("not enough tailroom (%d vs %d)", len, skb_tailroom(skb)));

		skb_put(skb, len);

#define ETH_P_CONTROL	0x0016		/* Card specific control frames , copy from linux/if_ether.h */
