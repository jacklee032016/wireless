/*
* $Id: radar.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
*/
...
static void __radar_device_reanimate(unsigned long arg)
{
...
void radar_init(struct ieee80211com *ic)
{
	memset (ic->ic_channelList, 0, sizeof (ic->ic_channelList));
	init_timer(&ic->ic_radar_reanimate);
	ic->ic_radar_reanimate.function = __radar_device_reanimate;
	ic->ic_radar_reanimate.data = (unsigned long) ic;
}

struct ieee80211_channel *radar_handle_interference(struct ieee80211com *ic)
{
	...
	{
		if (!isclr(ic->ic_chan_active, index) && (ic->ic_channelList[index] == 0 || 
		...
		/* channel blocked */
		if((ic->ic_channelList[index] != 0) && (ic->ic_channelList[index] < firstTimeout))
		{
