/*
* $Id: phy_rate_sample.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 */
...
char *phyRateSampleVersion = "1.2";
char *phyRateSampleDevInfo = "PHY RATE SAMPLE";
...
int ath_smoothing_rate = 95;	/* ewma percentage (out of 100) */
int ath_sample_rate = 10;	/* use x% of transmission time 
					 * sending at a different bit-rate */

static void ath_rate_ctl_reset(struct ath_softc *, struct ieee80211_node *);

/* Calculate the transmit duration of a frame. */
static unsigned calc_usecs_unicast_packet(struct ath_softc *sc, int length, int rix, int short_retries, int long_retries)
{
	...
	if (rts || cts)
	{
		...
		KASSERT(rt->info[cix].rateKbps, ("cix %d (%d) bad ratekbps %d mode %u", cix, rt->info[cix].dot11Rate, rt->info[cix].rateKbps, sc->sc_curmode));
...
void ath_rate_node_copy(struct ath_softc *sc, struct ath_node *dst, const struct ath_node *src)
{
 ...
 */
static __inline int best_rate_ndx(struct sample_node *sn, int size_bin, int require_acked_before)
{
	int x = 0;
        int best_rate_ndx = 0;
        int best_rate_tt = 0;
        for (x = 0; x < sn->num_rates; x++)
	{
		...
		if (tt <= 0 || (require_acked_before && !sn->stats[size_bin][x].packets_acked))
		{
			continue;
		}
			...
			if (change_rates)
			{
				if (best_ndx != sn->current_rate[size_bin])
				{
					DPRINTF(sc, "%s: %s size %d switch rate %d (%d/%d) -> %d (%d/%d) after %d packets mrr %d\n",
						...
						swjtu_mesh_mac_sprintf(an->an_node.ni_macaddr),
		      ...
		      struct ath_desc *ds, int shortPreamble, u_int8_t rix)
{
	...
	ath_hal_setupxtxdesc(sc->sc_ah, ds   , rateCode, 3	        /* series 1 */
			     , sn->rates[0].rateCode, 3	/* series 2 */
			     , 0, 0	                /* series 3 */    );
	
}

static void  update_stats(struct ath_softc *sc, struct ath_node *an, 
		  int frame_size,
		  int ndx0, int tries0,
		  int ndx1, int tries1,
		  int ndx2, int tries2,
		  ...
		  int short_tries, int tries, int status)
{
	...
	if (ndx0 == sn->current_sample_ndx[size_bin])
	{
		DPRINTF(sc, "%s: %s size %d sample rate %d tries (%d/%d) tt %d avg_tt (%d/%d) status %d\n", 
			phyRateSampleDevInfo, swjtu_mesh_mac_sprintf(an->an_node.ni_macaddr), 
...
void phy_rate_tx_complete(struct ath_softc *sc, struct ath_node *an, const struct ath_desc *ds)
{
	...
	if (sn->num_rates <= 0)
	{
		DPRINTF(sc, "Sample : %s %s no rates yet\n", swjtu_mesh_mac_sprintf(an->an_node.ni_macaddr), __func__);
		return;
	}

	mrr = sc->sc_mrretry && !(ic->ic_flags & IEEE80211_F_USEPROT) && !(frame_size > ic->ic_rtsthreshold) && ENABLE_MRR;


	if (sc->sc_mrretry && ds->ds_txstat.ts_status) {
		/* this packet failed */
		DPRINTF(sc, ": %s size %d rate/try %d/%d %d/%d %d/%d %d/%d status %s retries (%d/%d)\n", 
			phyRateSampleDevInfo,
			swjtu_mesh_mac_sprintf(an->an_node.ni_macaddr),
	...
	if (!mrr || !(ds->ds_txstat.ts_rate & HAL_TXSTAT_ALTRATE))
	{
		...
		{
			update_stats(sc, an, frame_size, ndx, long_tries,
				     0, 0,
				     0, 0,
				     0, 0,
...
{
	DPRINTF(sc, "%s: %s %s\n", phyRateSampleDevInfo, swjtu_mesh_mac_sprintf(an->an_node.ni_macaddr), __func__);
	if (isnew)
 ...
 */
static void ath_rate_ctl_reset(struct ath_softc *sc, struct ieee80211_node *ni)
{
	struct ieee80211com *ic = &sc->sc_ic;
	...
	if (sn->num_rates <= 0)
	{
		DPRINTF(sc, "%s: %s %s no rates (fixed %d) \n", phyRateSampleDevInfo, __func__, swjtu_mesh_mac_sprintf(ni->ni_macaddr), ic->ic_fixed_rate);
	...
	if (ic->ic_fixed_rate != -1)
	{
		...
		ni->ni_txrate = srate;
		DPRINTF(sc, "%s: %s %s fixed rate %d%sMbps\n", phyRateSampleDevInfo, __func__, swjtu_mesh_mac_sprintf(ni->ni_macaddr), 
...
static void ath_rate_cb(void *arg, struct ieee80211_node *ni)
{
        ath_rate_ctl_reset((struct ath_softc *) arg, ni);
}

/* Reset the rate control state for each 802.11 state transition. */
void phy_rate_newstate(struct ath_softc *sc, enum ieee80211_state newstate)
{
	struct ieee80211com *ic = &sc->sc_ic;
