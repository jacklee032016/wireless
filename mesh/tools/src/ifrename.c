...
{ 
  ...
  char			iwproto[MESHNAMSIZ + 1];	/* Wireless/protocol name */
...
typedef int (*mapping_get)(int			devFd,
			   const char *		ifname,
			   struct if_mapping *	target,
			   int			flag);
...
static int
	mapping_cmpfirmware(struct if_mapping *	ifnode,
			    struct if_mapping *	target);
static int
	mapping_getdriverbusinfo(int			devFd,
				 const char *		ifname,
				 struct if_mapping *	target,
				 int			flag);
static int
...
static int
	mapping_cmpirq(struct if_mapping *	ifnode,
		       struct if_mapping *	target);
static int
	mapping_getbaseaddrirq(int			devFd,
			       const char *		ifname,
			       struct if_mapping *	target,
			       int			flag);
static int
...
static int
	mapping_cmppcmciaslot(struct if_mapping *	ifnode,
			   struct if_mapping *		target);
static int
	mapping_getpcmciaslot(int			devFd,
			      const char *		ifname,
			      struct if_mapping *	target,
			      int			flag);
static int
 ...
 */
static int
if_takeover_name(int			devFd,
		 const char *		victimname)
{
  char		autoname[MESHNAMSIZ+1];
  int		len;
  struct ifreq	ifr;
  int		ret;
  ...
  if(len > (MESHNAMSIZ - 2))
    len = MESHNAMSIZ - 2;		/* Make sure we have at least two char */
  ...
  strncpy(ifr.ifr_name, victimname, MESHNAMSIZ); 
  strncpy(ifr.ifr_newname, autoname, MESHNAMSIZ); 

  /* Rename victim interface */
  ret = ioctl(devFd, SIOCSIFNAME, &ifr);
	    ...
	    char *		retname)
{
  ...
  /* Prepare request */
  bzero(&ifr, sizeof(struct ifreq));
  strncpy(ifr.ifr_name, oldname, MESHNAMSIZ); 
  strncpy(ifr.ifr_newname, newname, MESHNAMSIZ); 
  ...
  if(ret && (errno == EEXIST) && force_takeover)
    {
      /* Push things around */
      ret = if_takeover_name(devFd, newname);
      if(!ret)
	/* Second try */
	ret = ioctl(devFd, SIOCSIFNAME, &ifr);
    }
	       ...
	       int			linenum)
{
  ...
  if (strchr(ifnode->mac_filter, '*') != NULL)
  ...
  else
    {
      ...
      ifnode->mac_len = meshu_mac_aton(ifnode->mac_filter,
	       ...
	       int			flag)
{
  ...
  if((flag == HAS_MAC_FILTER) || verbose)
    {
      /* Convert to ASCII */
      meshu_mac_ntop(target->mac, target->mac_len,
	       ...
	       int			flag)
{
  ...
  return(mapping_getmac(devFd, ifname, target, flag));
}
			 ...
			 int			flag)
{
  ...
  /* Do it */
  ret = ioctl(devFd, SIOCETHTOOL, &ifr);
  if(ret < 0)
    {
		       ...
		       int			flag)
{
  ...
  /* Do it */
  ret = ioctl(devFd, SIOCGIFMAP, &ifr);
  if(ret < 0)
    {
		   ...
		   int			flag)
{
  ...
  strncpy(target->iwproto, wrq.u.name, MESHNAMSIZ);
  target->iwproto[MESHNAMSIZ] = '\0';
		...
		const char *	ifname)
{
  ...
  for(i = 0; i < SELECT_NUM; i++)
	...
	{
	  /* Extract selector */
	  selector_list[i].get_fn(devFd, ifname, target, selector_active[i]);
...
probe_mappings(int		devFd)
{
  ...
  for(ifnode = mapping_list; ifnode != NULL; ifnode = ifnode->next)
	...
	fprintf(stderr, "Probing : Trying to load interface [%s]\n",
       ...
       */
      strncpy(ifr.ifr_name, ifnode->ifname, MESHNAMSIZ);
      ioctl(devFd, SIOCGIFHWADDR, &ifr);
    }
}
      ...
      if(!strncasecmp(linebuf, "auto ", 5))
	{
	  ...
	  while(*p != '\0')
		...
		fprintf(stderr, "Probing : Trying to load interface [%s]\n",
	      ...
	      /* Load interface */
	      strncpy(ifr.ifr_name, p, MESHNAMSIZ);
	      ioctl(devFd, SIOCGIFHWADDR, &ifr);
 ...
 */
static int
process_rename(int	devFd,
	       char *	ifname,
	       char *	pattern)
{
  char		newname[MESHNAMSIZ+1];
  char		retname[MESHNAMSIZ+1];
  int		len;
  char *	star;
  ...
  if(if_set_name(devFd, ifname, newname, retname) < 0)
    {
	       ...
	       int	count)
{
  ...
  target = mapping_extract(devFd, ifname);
  if(target == NULL)
    return(-1);
  ...
  /* Change the name of the interface */
  if(if_set_name(devFd, target->ifname, mapping->ifname, retname) < 0)
    {
      fprintf(stderr, "Error: cannot change name of %s to %s: %s\n",
	       ...
	       int	count)
{
  ...
  /* Just do it */
  iw_enum_devices(devFd, &process_ifname, args, count);
     ...
     char *	argv[]) 
{
  ...
  if(ifname != NULL)
      ...
      if(new_name != NULL)
	{
	  ...
	  ret = process_rename(devFd, ifname, new_name);
	}
      else
	{
	   ...
	   */
	  print_newname = 1;
	  ret = process_ifname(devFd, ifname, NULL, 0);
	}
    }
  else
    {
