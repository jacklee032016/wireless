...
typedef struct wireless_iface
{
  ...
  /* Interface data */
  char			ifname[MESHNAMSIZ + 1];	/* Interface name */
  struct meshw_range	range;			/* Wireless static data */
  int			has_range;
} wireless_iface;
	   ...
	   char *	name)
{
  ...
  memset(name, 0, MESHNAMSIZ + 1);

  /* Get interface name */
  irq.ifr_ifindex = ifindex;
  if(ioctl(devFd, SIOCGIFNAME, &irq) < 0)
    ret = -1;
  else
    strncpy(name, irq.ifr_name, MESHNAMSIZ);
...
iw_get_interface_data(int	ifindex)
{
  ...
  /* Extract static data */
  if(index2name(devFd, ifindex, curr->ifname) < 0)
    {
      perror("index2name");
      free(curr);
      return(NULL);
    }
  curr->has_range = (meshwu_get_range_info(devFd, curr->ifname, &curr->range) >= 0);
 ...
 * Maybe we should have better formatting like meshwu_print_key...
 */
static char *
iw_hexdump(char *		buf,
	   size_t		buflen,
 ...
 */
static inline int
print_event_token(struct meshw_event *	event,		/* Extracted token */
		  struct meshw_range *	meshw_range,	/* Range info */
		  int			has_range)
{
  ...
  switch(event->cmd)
    ...
    case MESHW_IO_W_NWID:
      if(event->u.nwid.disabled)
	printf("Set NWID:off/any\n");
      else
	printf("Set NWID:%X\n", event->u.nwid.value);
      break;
    case MESHW_IO_W_FREQ:
	...
	if(has_range)
	  {
	    if(freq < KILO)
	      ...
	      channel = meshwu_channel_to_freq((int) freq, &freq, iw_range);
	    else
	      /* Convert frequency to channel if possible */
	      channel = meshwu_freq_to_channel(freq, iw_range);
	  }
	meshwu_print_freq(buffer, sizeof(buffer),
		      freq, channel, event->u.freq.flags);
	printf("%s %s\n", prefix, buffer);
      }
      break;
    case MESHW_IO_W_MODE:
      printf("Set Mode:%s\n",
	     meshwu_operation_mode[event->u.mode]);
      break;
    case MESHW_IO_W_ESSID:
    ...
    case MESHW_IO_R_AP_MAC:
      printf("New Access Point/Cell address:%s\n",
	     meshwu_sawap_ntop(&event->u.ap_addr, buffer));
      break;
    case MESHW_IO_R_SCAN:
      printf("Scan request completed\n");
      break;
    case MESHW_EV_TX_DROP:
      printf("Tx packet dropped:%s\n",
	     meshwu_saether_ntop(&event->u.addr, buffer));
      break;
      ...
      if(event->u.data.length >= sizeof(struct iw_michaelmicfailure))
	{
	  ...
	  printf("Michael MIC failure flags:0x%X src_addr:%s tsc:%s\n",
		 mf.flags,
		 meshwu_saether_ntop(&mf.src_addr, buffer2),
      ...
      if(event->u.data.length >= sizeof(struct iw_pmkid_cand))
	{
	  ...
	  printf("PMKID candidate flags:0x%X index:%d bssid:%s\n",
		 ...
		 meshwu_saether_ntop(&cand.bssid, buffer));
	}
      break;
      /* ----- junk ----- */
		   ...
		   int		len)
{
  ...
  meshwu_print_timeval(buffer, sizeof(buffer), &recv_time, &tz);

  meshwu_init_event_stream(&stream, data, len);
  do
    {
     ...
     char *	argv[])
{
  ...
  while((opt = getopt_long(argc, argv, "hv", long_opts, NULL)) > 0)
	...
	case 'v':
	  return(meshwu_print_version_info("iwevent"));
	  break;
