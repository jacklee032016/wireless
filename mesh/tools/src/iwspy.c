	       ...
	       int	count)
{
  ...
  char		buffer[(sizeof(struct meshw_quality) +
			sizeof(struct sockaddr)) * MESHW_MAX_SPY];
  char		temp[128];
  struct sockaddr *	hwa;
  ...
  wrq.u.data.length = MESHW_MAX_SPY;
  wrq.u.data.flags = 0;
  if(meshwu_get_ext(devFd, ifname, MESHW_IO_R_SPY, &wrq) < 0)
    {
  ...
  for(i = 0; i < n; i++)
    {
      ...
      meshwu_print_stats(temp, sizeof(temp), &qual[i], &range, has_range);
      printf("%s\n", temp);
    }
  ...
  if((n > 0) && (has_range) && (range.we_version_compiled > 11))
    {
      ...
      if(meshwu_get_stats(devFd, ifname, &stats, &range, has_range) >= 0)
	{
	  meshwu_print_stats(temp, sizeof(temp), &stats.qual, &range, has_range);
	  printf("    Link/Cell/AP      : %s\n", temp);
	  /* Display the static data */
	  meshwu_print_stats(temp, sizeof(temp),
 ...
 */
static int
get_spy_threshold(int		devFd,		/* The socket */
		  ...
		  int		count)		/* Args count */
{
  ...
  wrq.u.data.length = 1;
  wrq.u.data.flags = 0;
  if(meshwu_set_ext(devFd, ifname, MESHW_IO_R_SPY_THR, &wrq) < 0)
    {
      fprintf(stderr, "Interface doesn't support thresholds...\n");
      fprintf(stderr, "MESHW_IO_R_SPY_THR: %s\n", strerror(errno));
      return(-1);
    }
 ...
 */
static int
set_spy_info(int		devFd,		/* The socket */
	     char *		ifname,		/* Dev name */
	     char *		args[],		/* Command line args */
	     int		count)		/* Args count */
{
  ...
  struct sockaddr	hw_address[MESHW_MAX_SPY];
  ...
  if(!strcmp(args[0], "off"))
      ...
      if(!strcmp(args[0], "+"))
	{
	  ...
	  if(meshwu_get_ext(devFd, ifname, MESHW_IO_R_SPY, &wrq) < 0)
	    {
	      ...
	      fprintf(stderr, "MESHW_IO_R_SPY: %s\n", strerror(errno));
	      return(-1);
	    }
      ...
      while((i < count) && (nbr < MESHW_MAX_SPY))
	{
	  /* Get the address and check if the interface supports it */
	  if(meshwu_in_addr(devFd, ifname, args[i++], &(hw_address[nbr])) < 0)
	    continue;
	  nbr++;
	}
  ...
  wrq.u.data.flags = 0;
  if(meshwu_set_ext(devFd, ifname, MESHW_IO_W_SPY, &wrq) < 0)
    {
      fprintf(stderr, "Interface doesn't accept addresses...\n");
      fprintf(stderr, "MESHW_IO_W_SPY: %s\n", strerror(errno));
      return(-1);
    }
 ...
 */
static int
set_spy_threshold(int		devFd,		/* The socket */
		  char *	ifname,		/* Dev name */
		  char *	args[],		/* Command line args */
		  int		count)		/* Args count */
{
  ...
  wrq.u.data.length = 1;
  wrq.u.data.flags = 0;
  if(meshwu_set_ext(devFd, ifname, MESHW_IO_W_SPY_THR, &wrq) < 0)
    {
      fprintf(stderr, "Interface doesn't accept thresholds...\n");
      fprintf(stderr, "MESHW_IO_W_SPY_THR: %s\n", strerror(errno));
      return(-1);
    }
     ...
     char **	argv)
{
  ...
  if(argc == 1)
    iw_enum_devices(devFd, &print_spy_info, NULL, 0);
  else
	...
	goterr = meshwu_print_version_info("iwspy");
      else
	/* The device name must be the first argument */
	/* Name only : show spy list for that device only */
	if(argc == 2)
	  print_spy_info(devFd, argv[1], NULL, 0);
	else
	  /* Special commands */
	  if(!strcmp(argv[2], "setthr"))
	    goterr = set_spy_threshold(devFd, argv[1], argv + 3, argc - 3);
	  else
	    if(!strcmp(argv[2], "getthr"))
	      goterr = get_spy_threshold(devFd, argv[1], argv + 3, argc - 3);
	    else
	      /* Otherwise, it's a list of address to set in the spy list */
	      goterr = set_spy_info(devFd, argv[1], argv + 2, argc - 2);
