/*
* $Id: wlan_crypto.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
*/
 ...
 */
int ieee80211_crypto_newkey(struct ieee80211com *ic, int cipher, int flags, struct ieee80211_key *key)
{
#define	N(a)	(sizeof(a) / sizeof(a[0]))
	...
	{
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: invalid cipher %u\n", __func__, cipher);
		ic->ic_stats.is_crypto_badcipher++;
		return 0;
	}
	
	cip = wlanCiphers[cipher];
	if (cip == NULL)
	{
#if 0	
		/*
		...
		{
			IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: unregistered cipher %u, load module %s\n", __func__, cipher, cipher_modnames[cipher]);
...
#endif			
		{
			IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: unable to load cipher %u, module %s\n", 	__func__, cipher,	cipher < (int)N(cipher_modnames) ? cipher_modnames[cipher] : "<unknown>");
	...
	{
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: no h/w support for cipher %s, falling back to s/w\n",  __func__, cip->ic_name);
	 ...
	 */
	if (cipher == IEEE80211_CIPHER_TKIP &&  (ic->ic_caps & IEEE80211_C_TKIPMIC) == 0)
	{
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO,   "%s: no h/w support for TKIP MIC, falling back to s/w\n",  __func__);
...
again:
		...
		if (keyctx == NULL)
		{
			IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: unable to attach cipher %s\n", __func__, cip->ic_name);
	...
	if (key->wk_keyix == IEEE80211_KEYIX_NONE)
	{
		...
		if (key->wk_keyix == IEEE80211_KEYIX_NONE)
		{
			...
			if ((key->wk_flags & IEEE80211_KEY_SWCRYPT) == 0)
			{
				...
				IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: no h/w resources for cipher %s, falling back to s/w\n", __func__, cip->ic_name);
				oflags = key->wk_flags;
				flags |= IEEE80211_KEY_SWCRYPT;
			...
			IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: unable to setup cipher %s\n",  __func__, cip->ic_name);
			return 0;
		}
	}
	return 1;
 ...
 */
static int _ieee80211_crypto_delkey(struct ieee80211com *ic, struct ieee80211_key *key)
{
	u_int16_t keyix;
 ...
 */
int ieee80211_crypto_delkey(struct ieee80211com *ic, struct ieee80211_key *key)
{
	int status;
 ...
 */
void ieee80211_crypto_delglobalkeys(struct ieee80211com *ic)
{
	int i;
 ...
 *	ieee80211_key_update_end(ic);
 */
int ieee80211_crypto_setkey(struct ieee80211com *ic, struct ieee80211_key *key,
		const u_int8_t macaddr[IEEE80211_ADDR_LEN])
{
	...
	IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO,
	    ...
	    key->wk_flags, swjtu_mesh_mac_sprintf(macaddr),
	    key->wk_keyrsc, key->wk_keytsc, key->wk_keylen);
	...
	{
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: cipher %s rejected key index %u len %u flags 0x%x\n",
	...
	if (key->wk_keyix == IEEE80211_KEYIX_NONE) {
		...
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO,  "%s: no key index; should not happen!\n", __func__);
 ...
 */
struct ieee80211_key *ieee80211_crypto_encap(struct ieee80211com *ic, struct ieee80211_node *ni, struct sk_buff *skb)
{
	    ...
	    ni->ni_ucastkey.wk_cipher == &ieee80211_cipher_none)
	{
		if (ic->ic_def_txkey == IEEE80211_KEYIX_NONE)
		{
			IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "[%s] no default transmit key (%s) deftxkey %u\n",
			    swjtu_mesh_mac_sprintf(wh->i_addr1), __func__, ic->ic_def_txkey);
	...
	if (skb_headroom(skb) < (int)cip->ic_header)
	{
		 ...
		 */
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_CRYPTO, "%s: Malformed packet for cipher %s; headroom %u\n",	__func__, cip->ic_name, skb_headroom(skb));
 ...
 */
struct ieee80211_key *ieee80211_crypto_decap(struct ieee80211com *ic, struct ieee80211_node *ni, struct sk_buff *skb)
{
