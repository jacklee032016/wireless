/*
* $Id: wlan_input_data.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
 */
...
static struct sk_buff *__ieee80211_decap(struct ieee80211com *ic, struct sk_buff *skb)
{
	...
	switch (wh.i_fc[1] & IEEE80211_FC1_DIR_MASK)
	{
		...
		case IEEE80211_FC1_DIR_DSTODS:
			/* not yet supported */
			IEEE80211_DISCARD(ic, IEEE80211_MSG_ANY, &wh, "data", "%s", "DS to DS not supported");
			dev_kfree_skb(skb);
			return NULL;
	}
 ...
 */
static struct sk_buff *__ieee80211_defrag(struct ieee80211com *ic, struct ieee80211_node *ni, struct sk_buff *skb)
{
		...
		if (more_frag)
		{
				...
				sizeof(struct ieee80211_frame))
			{
				...
				skbfrag = skb_copy_expand(skb, 0,
					(ic->ic_dev->mtu + sizeof(struct ieee80211_frame))
	...
	{/* concatenate */
		...
		memcpy(skbfrag->tail, skb->data + sizeof(struct ieee80211_frame), skb->len - sizeof(struct ieee80211_frame) );
...
int _ieee80211_input_data(struct ieee80211com *ic, struct sk_buff *skb, struct ieee80211_node *ni, int rssi, u_int32_t rstamp)
{
...
ktrace;
	...
	if (skb->len < hdrsize)
	{
		IEEE80211_DISCARD_MAC(ic, IEEE80211_MSG_ANY, ni->ni_macaddr, NULL, "data too short: len %u, expecting %u", skb->len, hdrsize);
...
ktrace;
	...
	switch (ic->ic_opmode)
	{
		...
		case IEEE80211_M_STA:
		{
			...
			if ((dev->flags & MESHF_MULTICAST) && IEEE80211_IS_MULTICAST(wh->i_addr1) &&   IEEE80211_ADDR_EQ(wh->i_addr3, ic->ic_myaddr))
			{
				/*
		...
		case IEEE80211_M_HOSTAP:
		{
			...
			if (ni == ic->ic_bss)
			{
				...
				if (ni != NULL)
				{
					IEEE80211_SEND_MGMT(ic, ni, IEEE80211_FC0_SUBTYPE_DEAUTH, IEEE80211_REASON_NOT_AUTHED);
			...
			if (ni->ni_associd == 0)
			{
				...
				IEEE80211_SEND_MGMT(ic, ni, IEEE80211_FC0_SUBTYPE_DISASSOC, IEEE80211_REASON_NOT_ASSOCED);
				ic->ic_stats.is_rx_notassoc++;
				goto err;
			}
	...
	if (wh->i_fc[1] & IEEE80211_FC1_WEP)
	{
		if ((ic->ic_flags & IEEE80211_F_PRIVACY) == 0)
		{
			 ...
			 */
			IEEE80211_DISCARD(ic, IEEE80211_MSG_INPUT,  wh, "WEP", "%s", "PRIVACY off");
	...
	{/* muilcast frame can not be fragment */
		skb = __ieee80211_defrag(ic, ni, skb);
		if (skb == NULL)
	...
	{
		IEEE80211_DISCARD_MAC(ic, IEEE80211_MSG_INPUT, ni->ni_macaddr, "data", "%s", "demic error");
	...
	if (!ieee80211_node_is_authorized(ni))
	{
		...
		{
			IEEE80211_DISCARD_MAC(ic, IEEE80211_MSG_INPUT, eh->ether_shost, "data", "unauthorized port: ether type 0x%x len %u",  eh->ether_type, skb->len);
	...
	else
	{
		...
		if ((ic->ic_flags & IEEE80211_F_DROPUNENC) && key == NULL && eh->ether_type != __constant_htons(ETHERTYPE_PAE))
		{
			/* Drop unencrypted frames. */
			IEEE80211_DISCARD_MAC(ic, IEEE80211_MSG_INPUT, eh->ether_shost, "data", "unencrypted: ether type 0x%x len %u", eh->ether_type, skb->len);
...
ktrace;
	...
	if (ic->ic_opmode == IEEE80211_M_HOSTAP &&  (ic->ic_flags & IEEE80211_F_NOBRIDGE) == 0)
	{
		...
		else
		{
			...
			if (ni1 != NULL && ni1 != ic->ic_bss)
			{
				...
				{
					IEEE80211_DISCARD_MAC(ic, IEEE80211_MSG_INPUT, eh->ether_shost, "data", "bridge: node %s (aid: %d) not authorized", swjtu_mesh_mac_sprintf(ni1->ni_macaddr), ni1->ni_associd);
				}
				/* XXX statistic? */
			}
			else if (!IEEE80211_ADDR_EQ(eh->ether_dhost, ic->ic_bss->ni_bssid))
			{
				IEEE80211_DISCARD_MAC(ic, IEEE80211_MSG_INPUT, eh->ether_shost, "data", "bridge: dest node %s not found", swjtu_mesh_mac_sprintf(eh->ether_dhost));
			}
		...
		{
			len = skb1->len;
			skb1->dev = (struct net_device *)dev;
			skb1->mac.raw = skb1->data;
			skb1->nh.raw = skb1->data + sizeof(struct ether_header);
			skb1->protocol = __constant_htons(ETH_P_802_2);
#if 0			
			meshdev_queue_xmit(skb1);			// NB: send directly to iface
#endif
