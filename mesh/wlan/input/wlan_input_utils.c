/*
* $Id: wlan_input_utils.c,v 1.1.1.1 2006/11/30 17:00:40 lizhijie Exp $
*/
...
{
	printf("[%s] %s%s on chan %u (bss chan %u) ",   swjtu_mesh_mac_sprintf(mac), isnew ? "new " : "",
	...
	if (isnew)
	{
		printf("[%s] caps 0x%x bintval %u erp 0x%x", swjtu_mesh_mac_sprintf(mac), capinfo, bintval, erp);
		if (country)
		{
	...
	const char *type, const char *fmt, ...)
{
	 ...
	 * because swjtu_mesh_mac_sprintf may be used there as well
	...
	printf("[%s] discard ", swjtu_mesh_mac_sprintf(ieee80211_getbssid(ic, wh)));
	if (type != NULL)
		printf("%s frame, ", type);
	else
		printf("frame, ");
	...
	const char *type, const char *fmt, ...)
{
	...
	printf("[%s] discard ", swjtu_mesh_mac_sprintf(mac));
	if (type != NULL)
		printf("%s frame, ", type);
	else
		printf("frame, ");
	...
	u_int8_t *rates, u_int8_t *xrates, int flags)
{
	...
	if (xrates != NULL)
	{
		...
		if (rs->rs_nrates + nxrates > IEEE80211_RATE_MAXSIZE)
		{
			...
			IEEE80211_DPRINTF(ic, IEEE80211_MSG_XRATE, "[%s] extended rate set too large;" " only using %u of %u rates\n", swjtu_mesh_mac_sprintf(ni->ni_macaddr), nxrates, xrates[1]);
...
void ieee80211_node_pwrsave(struct ieee80211_node *ni, int enable)
{
	...
	if (enable)
	{
		...
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_POWER, "[%s] power save mode on, %u sta's in ps mode\n", swjtu_mesh_mac_sprintf(ni->ni_macaddr), ic->ic_ps_sta);
		return;
	}
	...
	IEEE80211_DPRINTF(ic, IEEE80211_MSG_POWER, "[%s] flush ps queue, %u packets queued\n", swjtu_mesh_mac_sprintf(ni->ni_macaddr), IEEE80211_NODE_SAVEQ_QLEN(ni));
...
void ieee80211_recv_pspoll(struct ieee80211com *ic, 	struct ieee80211_node *ni, struct sk_buff *skb0)
{
	...
	if (ni->ni_associd == 0)
	{
		IEEE80211_DISCARD(ic, IEEE80211_MSG_POWER | IEEE80211_MSG_DEBUG, (struct ieee80211_frame *) wh, "ps-poll", "%s", "unassociated station");
		ic->ic_stats.is_ps_unassoc++;
		
		IEEE80211_SEND_MGMT(ic, ni, IEEE80211_FC0_SUBTYPE_DEAUTH, IEEE80211_REASON_NOT_ASSOCED);
		return;
	}
	...
	{
		IEEE80211_DISCARD(ic, IEEE80211_MSG_POWER | IEEE80211_MSG_DEBUG, (struct ieee80211_frame *) wh, "ps-poll", "aid mismatch: sta aid 0x%x poll aid 0x%x",  ni->ni_associd, aid);
	...
	if (skb == NULL)
	{
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_POWER,  "[%s] recv ps-poll, but queue empty\n",   swjtu_mesh_mac_sprintf(wh->i_addr2));
	...
	if (qlen != 0)
	{
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_POWER, "[%s] recv ps-poll, send packet, %u still queued\n",   swjtu_mesh_mac_sprintf(ni->ni_macaddr), qlen);
	...
	{
		IEEE80211_DPRINTF(ic, IEEE80211_MSG_POWER, "[%s] recv ps-poll, send packet, queue empty\n", swjtu_mesh_mac_sprintf(ni->ni_macaddr));
