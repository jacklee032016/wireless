/*
* $Id: ip_netfilter.c,v 1.1.1.1 2006/11/30 17:00:19 lizhijie Exp $
*/
...
#if 0
	result = engine->input_packet(engine, skb);
	switch(result)
	{
...
#if 0
	return __aodv_packet_tx(engine, iph);
#else
	routeDev = route_dev_lookup( (*skb)->dev );
	if(!routeDev)
