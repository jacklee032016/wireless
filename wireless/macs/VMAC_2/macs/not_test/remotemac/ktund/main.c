...
static int
ktund_tunnel_rx(vmac_mac_t *remotemac)
{
    int pkt_len, len;
    struct sk_buff *skb;
    struct ktund_packet_hdr *pkt_hdr;
	...
	remotemac->mac_tx(remotemac->mac_private, skb, 0);
...
ktund_tunnel_main(void *v)
{
...
#endif
#if 0
	athphy = vmac_get_phy_by_name("athphy0");
	if (!athphy)
	    athphy = vmac_get_phy( vmac_layer_new_instance("athphy") );
	if (!athphy) {
...
#endif
	remotemac = vmac_get_mac_by_name("remotemac0");
	if (!remotemac)
	    remotemac = vmac_get_mac( vmac_layer_new_instance("remotemac") );
	if (!remotemac) {
	    printk(NAME "%s error couldn't get mac instance!\n", __func__);
	    module_put(THIS_MODULE);
	    return 0;
	}
	remotemac->set_rx_func(remotemac->mac_private, ktund_tunnel_tx, 0);
	//remotemac->attach_phy(remotemac->mac_private, athphy);
	//athphy->attach_mac(athphy->phy_private, remotemac);
	...
	remotemac->set_rx_func(remotemac->mac_private, 0, 0);
	//remotemac->detach_phy(remotemac->mac_private);
	//athphy->detach_mac(athphy->phy_private);

	/* decrement reference count */
	//dev_put(ath_dev);
	//vmac_free_phy(athphy);
	vmac_free_mac(remotemac);
