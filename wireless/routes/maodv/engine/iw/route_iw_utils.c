/*
* $Id: route_iw_utils.c,v 1.1.1.1 2006/11/30 17:00:19 lizhijie Exp $
*/
...
trace;
	...
	ROUTE_LIST_FOR_EACH(dev, engine->devs, list)
	{
...
#endif		
		{
			...
			for (i = 0; i < n; i++)
			{
				aodv_neigh_update_link(engine, hwa[i].sa_data, (u_int8_t) qual[i].noise - qual[i].level);//level);// - 0x100);
			}
		}
	}
