/*
* $Id: kroute.c,v 1.1.1.1 2006/11/30 17:00:19 lizhijie Exp $
*/
...
int aodv_insert_kroute(mesh_route_t *route)
{
	...
	if (( krtentry = __create_kroute_entry(route->ip, route->nextHop, route->netmask, route->dev->name )) == NULL)
	{
		return -1;
	}
	...
	if (error<0)
	{
		...
		printk("Next Hop %s ", inet_ntoa(route->nextHop) );
